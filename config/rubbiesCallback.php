...
if($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    $accountNumber = $response->craccount;
	...
	$currencyCode = "";
	$recipient = "From:- ".$response->originatorname;
	$recipient .= ", Account Number: ".$response->originatoraccountnumber;
	$recipient .= ", Bank Name: ".$fetch_bank['bankname']." | ".$bankCode;
	$date_time = date("Y-m-d H:i:s");
		
	$search_vacct = mysqli_query($link, "SELECT * FROM virtual_account WHERE account_number = '$accountNumber'");
	$getvnum = mysqli_num_rows($search_vacct);
	$fetch_vacct = mysqli_fetch_array($search_vacct);
	
	$search_pacct = mysqli_query($link, "SELECT * FROM pool_account WHERE account_number = '$accountNumber'");
	$getpnum = mysqli_num_rows($search_pacct);
	$fetch_pacct = mysqli_fetch_array($search_pacct);

	$search_tacct = mysqli_query($link, "SELECT * FROM till_virtual_account WHERE account_number = '$accountNumber'");
	...
	if($wunique == 0 && $punique == 0 && $tunique == 0){
		...
		($getvnum == 1 && $getpnum == 0 && $gettnum == 0 && $wunique == 0) ? mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$originator','$paymentReference','$recipient','$amountPaid','','Credit','$currencyCode','RUBBIES','','pending','$date_time','$initiator','','')") : "";
		($getvnum == 0 && $getpnum == 1 && $gettnum == 0 && $punique == 0) ? mysqli_query($link, "INSERT INTO pool_history VALUES(null,'$originator','$paymentReference','$recipient','$amountPaid','','Credit','$currencyCode','RUBBIES','','pendingpool','$date_time','$initiator','')") : "";
		($getvnum == 0 && $getpnum == 0 && $gettnum == 1 && $tunique == 0) ? mysqli_query($link, "INSERT INTO fund_allocation_history VALUES(null,'$paymentReference','$originator','$initiator','$branch','$recipient','$initiator','$amountPaid','Credit','RUBBIES','$currencyCode','','$recipient','pendingtill','$date_time')") : "";

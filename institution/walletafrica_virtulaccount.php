<?php
$search_vaccount = mysqli_query($link, "SELECT * FROM virtual_account WHERE userid = '$iuid' AND gateway_name = 'walletafrica' AND status = 'ACTIVE'");
if(mysqli_num_rows($search_vaccount) == 0){
    ...
    $result = array();
    $result2 = array();
    $fullname_splitter = (explode(' ',$iname));
    $fname = $fullname_splitter[0];
    $lname = $fullname_splitter[1];
    $customerEmail = $myiemail_addrs;
    ...
    if($ivirtual_phone_no != ""){
    	...
    	if($result2['Response']['ResponseCode'] === "200"){
    	    ...
    	    mysqli_query($link, "INSERT INTO virtual_account VALUES(null,'$ivirtual_phone_no','$iuid','$accountName','$accountNumber','$bankName','ACTIVE','$datetime','walletafrica')");
    	    mysqli_query($link, "UPDATE user SET virtual_number = '$ivirtual_phone_no', virtual_acctno = '$accountNumber' WHERE id = '$iuid'");
    	...
    	if($result['Response']['ResponseCode'] === "200"){
    	    ...
    	    mysqli_query($link, "INSERT INTO virtual_account VALUES(null,'$unique_phone_no','$iuid','$accountName','$accountNumber','$bankName','ACTIVE','$datetime','walletafrica')");
    	    mysqli_query($link, "UPDATE user SET virtual_number = '$unique_phone_no', virtual_acctno = '$accountNumber' WHERE id = '$iuid'");

...
if($startDate != "" && $endDate != "" && $pmtType != "" && $filterBy != "" && $pmtType != "All"){
    ...
    ($individual_transaction_records != "1" && $branch_transaction_records != "1") ? $query .= "SELECT * FROM ledger_withdrawal_request 
...
foreach($result as $row)
{
 ...
 $sub_array[] = $row['acn'];
 $sub_array[] = ($fetch_borr['lname'] == "") ? "---" : $fetch_borr['lname'].' '.$fetch_borr['fname'];
 $sub_array[] = $row['phone'];
...
{
 ($individual_transaction_records != "1" && $branch_transaction_records != "1") ? $query = "SELECT * FROM ledger_withdrawal_request WHERE companyid = '$institution_id'" : "";
 ($individual_transaction_records === "1" && $branch_transaction_records != "1") ? $query = "SELECT * FROM ledger_withdrawal_request WHERE companyid = '$institution_id' AND acct_officer = '$iuid'" : "";
 ($individual_transaction_records != "1" && $branch_transaction_records === "1") ? $query = "SELECT * FROM ledger_withdrawal_request WHERE companyid = '$institution_id' AND sbranchid = '$isbranchid'" : "";
 $statement = $connect->prepare($query);
 $statement->execute();

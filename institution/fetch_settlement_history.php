...
$column = array('id', 'Status', 'Reference', 'Vendor Name', 'Vendor Contact', 'Request Amount', 'Destination Channel', 'A/c Details', 'DateTime');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all" && $filterBy != "Pending" && $filterBy != "Settled" && $filterBy != "Declined"){
    ...
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND vendorid = '$filterBy'
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "Declined"){
    ...
    $query .= "SELECT * FROM manual_investsettlement 
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND status = 'Declined'
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "Settled"){
    ...
    $query .= "SELECT * FROM manual_investsettlement 
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND status = 'Settled'
    ";
...
{
 $query = "SELECT * FROM manual_investsettlement WHERE companyid = '$institution_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

...
$instType = (startsWith($institution_id, "INST") ? "Institution" : (startsWith($institution_id, "AGT") ? "Agent" : "Merchant"));
...
<div class="columns">
  <ul class="price">
    ...
    <li class="bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><b><?php echo strtoupper($fetch_subplan['plan_name']); ?><?php echo ($isub_plan_code === $fetch_subplan['plan_code']) ? '<label class="bg-'.(($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']).'">Active</label>' : ''; ?></b><br><b style="font-size:20px;"><?php echo $icurrency.number_format($fetch_subplan['amount_per_months'],2,'.',','); ?></b> /<?php echo ($demo_type == 'day' && $fetch_subplan['amount_per_months'] == '0' ? 'day(s)' : ($demo_type == 'month' && $fetch_subplan['amount_per_months'] == '0' ? 'month(s)' : 'month(s)')); ?></li>
    ...
    <li class="bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>">
      <a href="finalize_saassub_upgrade.php?id=<?php echo $_SESSION['tid']; ?>&&mid=NDIw&&pcode=<?php echo $fetch_subplan['plan_code']; ?>"><button type="button" class="btn bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"> <i class="fa fa-upload">&nbsp;Upgrade Now</i></button></a>
    </li>
  </ul>
</div>

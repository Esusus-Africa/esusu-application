...
$column = array('Id', 'Branch', 'Account Officer', 'Account Number', 'Account Name', 'Bank Name', 'Phone', 'Email', 'Card ID', 'Wallet Balance', 'Status');
...
if($filterBy != "" && $filterBy != "all1" && $filterBy != "all2"){
    ...
    WHERE id = '$filterBy' AND created_by = '$institution_id'
    ";
...
if($filterBy != "" && $filterBy === "all1"){
    ...
    $query .= "SELECT * FROM user 
    WHERE created_by = '$institution_id' AND reg_type = 'agent' AND virtual_acctno != '' AND card_id != 'NULL' AND card_reg = 'Yes'
    ";
...
if($filterBy != "" && $filterBy === "all2"){
    ...
    $query .= "SELECT * FROM user 
    WHERE created_by = '$institution_id' AND reg_type = 'corporate' AND virtual_acctno != '' AND card_id != 'NULL' AND card_reg = 'Yes'
    ";
...
{
 $query = "SELECT * FROM user WHERE created_by = '$institution_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

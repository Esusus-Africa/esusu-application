...
$column = array('id', 'S/No', 'Operator-in-charge', 'Branch', 'Total Service Amount', 'Total Balance', 'Total Paid', 'NINSlip Status', 'IDCard Status');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all"){
    ...
    $query .= "SELECT DISTINCT(staffid), branchid FROM enrollees 
...
foreach($result as $row)
{
 ...
 $selSA = mysqli_query($link, "SELECT SUM(amount) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer'");
 $rowSA = mysqli_fetch_array($selSA);

 //TOTAL BALANCE LEFT
 $selBL = mysqli_query($link, "SELECT SUM(balance) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer'");
 $rowBL = mysqli_fetch_array($selBL);

 //PENDING SLIP STATUS
 $selPSS = mysqli_query($link, "SELECT COUNT(ninSlipStatus) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer' AND ninSlipStatus = 'Pending'");
 $rowPSS = mysqli_fetch_array($selPSS);
 //COMPLETED SLIP STATUS
 $selCSS = mysqli_query($link, "SELECT COUNT(ninSlipStatus) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer' AND ninSlipStatus = 'Completed'");
 $rowCSS = mysqli_fetch_array($selCSS);
 //COLLECTED SLIP STATUS
 $selCOLSS = mysqli_query($link, "SELECT COUNT(ninSlipStatus) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer' AND ninSlipStatus = 'Collected'");
 $rowCOLSS = mysqli_fetch_array($selCOLSS);

 //PENDING IDCARD STATUS
 $selPIDS = mysqli_query($link, "SELECT COUNT(idCardStatus) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer' AND idCardStatus = 'Pending'");
 $rowPIDS = mysqli_fetch_array($selPIDS);
 //COMPLETED IDCARD STATUS
 $selCIDS = mysqli_query($link, "SELECT COUNT(idCardStatus) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer' AND idCardStatus = 'Completed'");
 $rowCIDS = mysqli_fetch_array($selCIDS);
 //COLLECTED IDCARD STATUS
 $selCOLIDS = mysqli_query($link, "SELECT COUNT(idCardStatus) FROM enrollees WHERE createdAt BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id' AND staffid = '$acct_officer' AND idCardStatus = 'Collected'");
...
{
 $query = "SELECT DISTINCT(staffid), branchid FROM enrollees WHERE date_time BETWEEN '$startDate' AND '$endDate' AND companyid = '$institution_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

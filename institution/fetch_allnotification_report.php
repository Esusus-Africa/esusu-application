...
$column = array('id', 'RefID', 'Vendor', 'Customer Name', 'Plan Code', 'Sub. Code', 'Plan Name', 'Amount Paid', 'Bank Details', 'Status', 'Date/Time', 'Action');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all" && $filterBy != "Pending" && $filterBy != "Approved" && $filterBy != "Declined"){
    ...
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND merchantid = '$institution_id' AND (vendorid = '$filterBy' OR customerid = '$filterBy')
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "Declined"){
    ...
    $query .= "SELECT * FROM investment_notification 
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND merchantid = '$institution_id' AND status = 'Declined'
    ";
...
foreach($result as $row)
{
 ...
 $sub_array[] = "<div class='btn-group'>
                    ...
                    <li><p><a href='walletVerifiedInfo.php?id=".$_SESSION['tid']."&&mid=NDA0&&uid=".$acct_owner."' class='btn btn-default btn-flat' target='_blank'><i class='fa fa-search'>&nbsp;View KYC</i></a></p></li>
                    </ul>
                    </div>
                </div>";
 $data[] = $sub_array;
...
{
 $query = "SELECT * FROM investment_notification WHERE merchantid = '$institution_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

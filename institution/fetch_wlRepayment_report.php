...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all" && $filterBy != "all1" && $filterBy != "all2"){
    ...
    $query .= "SELECT * FROM wallet_loan_repayment 
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "all1"){
    ...
    $query .= "SELECT * FROM wallet_loan_repayment 
    WHERE pay_date BETWEEN '$startDate' AND '$endDate' AND branchid = '$institution_id' AND (tid = '$iuid' OR account_no = '$ivirtual_acctno')
    ";
...
foreach($result as $row)
{
 ...
 $utc_date = DateTime::createFromFormat('Y-m-d G:i:s',date("Y-m-d h:i:s", strtotime($row['pay_date'])),new DateTimeZone(date_default_timezone_get()));

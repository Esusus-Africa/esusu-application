...
$column = array('id', 'Account Name', 'RefID', 'Recipient', 'Purpose', 'Credit', 'Balance', 'Debit', 'Status', 'DateTime');
...
foreach($result as $row)
{
 ...
 $sub_array[] = "<span style='color: green;'><b>".($recipient === $filterBy && $row['credit'] == "" ? $row['currency'].number_format($row['debit'],2,'.',',') : (($recipient === $filterBy || $initiator === $filterBy) && $row['credit'] != "" ? $row['currency'].number_format($row['credit'],2,'.',',') : '---'))."</b></span>";

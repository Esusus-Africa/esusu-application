...
$column = array('id', 'Action', 'Type', 'Vendor Name', 'Customer Name', 'Plan Name', 'Subscription Code', 'Amount', 'Duration', 'Status', 'Activated By', 'Activation Date', 'Maturity Date','Expected Amount');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "all"){
    ...
    ($all_product_subscription === "1" && $individual_product_subscription != "1") ? $query .= "SELECT * FROM savings_subscription 
...
foreach($result as $row)
{
 ...
 $search_trans = mysqli_query($link, "SELECT SUM(amount), invoice_code FROM all_savingssub_transaction WHERE reference_no = '$reference_no' OR subscription_code = '$scode'");
 $fetch_trans = mysqli_fetch_array($search_trans);
 $txid = $fetch_trans['invoice_code'];
 $total_invested = $fetch_trans['SUM(amount)'];

 $search_transnum = mysqli_query($link, "SELECT * FROM all_savingssub_transaction WHERE reference_no = '$reference_no' OR subscription_code = '$scode'");
 ...
 $sub_array[] = "<div class='btn-group'>
                    ...
                    ".(($sstatus == "Approved" && $sub_type == "Manual"/**&& $fetchTrans_num != $duratn*/) ? "<li><p><a href='stop_msub.php?tid=".$_SESSION['tid']."&&scode=".$scode."&&mid=".base64_encode('1000')."' class='btn btn-default btn-flat'><i class='fa fa-times'>&nbsp;Stop Subscription</i></a></p></li>" : '')."
                    ".(($sstatus == "Approved" && $sub_type == "Manual" && $fetchTrans_num != $duratn && $current_stage < $furture_stage) ? "<li><p><a href='make_payment.php?tid=".$_SESSION['tid']."&&id=".$row['id']."&&acn=".$acn."&&mid=".base64_encode('1000')."&&pcode=".$newplan_code."' class='btn btn-default btn-flat'><i class='fa fa-download'>&nbsp;Make Deposit</i></a></p></li>" : '')."
                    ".(($irole == "agent_manager" && $irole == "institution_super_admin" && $irole == "merchant_super_admin") ? "<li><p><a href='walletVerifiedInfo.php?tid=".$_SESSION['tid']."&&id=".$row['id']."&&uid=".$acn."&&mid=".base64_encode('1000')."&&pcode=".$newplan_code."' target='_blank' class='btn btn-default btn-flat'><i class='fa fa-search'>&nbsp;View KYC / Document</i></a></p></li>" : '')."
                    </ul>
                    </div>
                </div>";
 $sub_array[] = "<b>".$row['rec_type']."</b>";
...
{
($all_product_subscription === "1" && $individual_product_subscription != "1") ? $query = "SELECT * FROM savings_subscription WHERE companyid = '$institution_id'" : "";
($all_product_subscription != "" && $individual_product_subscription === "1") ? $query = "SELECT * FROM savings_subscription WHERE companyid = '$institution_id' AND agentid = '$iuid'" : "";
 $statement = $connect->prepare($query);
 $statement->execute();

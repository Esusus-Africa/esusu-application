...
$column = array('id', 'action', 'sNo', 'branch', 'staffName', 'accountId', 'name', 'acctType', 'regType', 'phone', 'Last Update', 'Opening Date', 'ledgerBalance', 'walletBalance', 'status');
...
if($startDate != "" && $endDate != "" && $byCustomer == "" && $filterBy != ""){
    ...
    WHERE (acct_opening_date BETWEEN '$startDate' AND '$endDate'  AND branchid = '$institution_id' AND status = 'queue' AND (sbranchid = '$filterBy' OR lofficer = '$filterBy'))
     " : "";
     
    ($individual_customer_records === "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE (acct_opening_date BETWEEN '$startDate' AND '$endDate'  AND branchid = '$institution_id' AND lofficer = '$iuid' AND status = 'queue')
     " : "";
     
    ($individual_customer_records === "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE (acct_opening_date BETWEEN '$startDate' AND '$endDate'  AND branchid = '$institution_id' AND sbranchid = '$isbranchid' AND status = 'queue')
     " : "";
...
if($startDate == "" && $endDate == "" && $filterBy == "" && $byCustomer != ""){
    ...
    ($individual_customer_records != "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE account =  '$byCustomer' AND branchid = '$institution_id' AND status = 'queue'
     " : "";
     
    ($individual_customer_records === "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE account =  '$byCustomer' AND branchid = '$institution_id' AND lofficer = '$iuid' AND status = 'queue'
     " : "";
     
    ($individual_customer_records != "1" && $branch_customer_records === "1") ? $query .= "SELECT * FROM borrowers 
    WHERE account =  '$byCustomer' AND branchid = '$institution_id' AND sbranchid = '$isbranchid' AND status = 'queue'
     " : "";
...
if($startDate == "" && $endDate == "" && $filterBy != "All" && $filterBy != "" && $byCustomer == ""){
    ...
    ($individual_customer_records != "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE branchid = '$institution_id' AND status = 'queue' AND (sbranchid = '$filterBy' OR lofficer = '$filterBy')
     " : "";
     
    ($individual_customer_records === "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE branchid = '$institution_id' AND lofficer = '$iuid' AND status = 'queue'
     " : "";
     
    ($individual_customer_records != "1" && $branch_customer_records === "1") ? $query .= "SELECT * FROM borrowers 
    WHERE branchid = '$institution_id' AND sbranchid = '$isbranchid' AND status = 'queue'
     " : "";
...
if($startDate == "" && $endDate == "" && $filterBy == "All" && $byCustomer == ""){
    ...
    ($individual_customer_records != "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE branchid = '$institution_id' AND status = 'queue'
     " : "";
...
if($startDate != "" && $endDate != "" && $filterBy == "" && $byCustomer == ""){
    ...
    ($individual_customer_records != "1" && $branch_customer_records != "1") ? $query .= "SELECT * FROM borrowers 
    WHERE acct_opening_date BETWEEN '$startDate' AND '$endDate' AND branchid = '$institution_id' AND status = 'queue'
     " : "";
...
foreach($result as $row)
{
 ...
 $sub_array[] = ($acct_status == "Activated" ? "<span class='label bg-blue'>Active</span>" : ($acct_status == "Closed" ? "<span class='label bg-red'>Closed</span>" : "<span class='label bg-orange'>".$acct_status."</span>"));
 $data[] = $sub_array;
}
...
{
 ($individual_transaction_records != "1" && $branch_transaction_records != "1") ? $query = "SELECT * FROM borrowers WHERE branchid = '$institution_id' AND status = 'queue'" : "";
 ($individual_transaction_records === "1" && $branch_transaction_records != "1") ? $query = "SELECT * FROM borrowers WHERE branchid = '$institution_id' AND lofficer = '$iuid' AND status = 'queue'" : "";
 ($individual_transaction_records != "1" && $branch_transaction_records === "1") ? $query = "SELECT * FROM borrowers WHERE branchid = '$institution_id' AND sbranchid = '$isbranchid' AND status = 'queue'" : "";
 $statement = $connect->prepare($query);
 $statement->execute();

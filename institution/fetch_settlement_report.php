...
$column = array('id', 'Action', 'TID/TraceID', 'Operator', 'Channel', 'Pending Balance', 'Settled Amount', 'Transfer Balance', 'Status', 'Date/Time');
...
if($startDate != "" && $endDate != "" && $transType != "" && $transType != "all" && $transType != "Pending" && $transType != "Settled"){
    ...
    WHERE dateCreated BETWEEN '$startDate' AND '$endDate' AND terminal_status = 'Assigned' AND merchant_id = '$institution_id' AND (terminal_id = '$transType' OR tidoperator = '$transType' OR trace_id = '$transType')
     ";
...
if($startDate != "" && $endDate != "" && $transType != "" && $transType == "all"){
    ...
    $query .= "SELECT * FROM terminal_reg
    WHERE dateCreated BETWEEN '$startDate' AND '$endDate' AND terminal_status = 'Assigned' AND merchant_id = '$institution_id'";
...
if($startDate != "" && $endDate != "" && $transType != "" && $transType == "Pending"){
    ...
    $query .= "SELECT * FROM terminal_reg
    WHERE dateCreated BETWEEN '$startDate' AND '$endDate' AND merchant_id = '$institution_id' AND terminal_status = 'Assigned' AND (pending_balance > '0.0' OR pending_balance > 0)";
...
if($startDate != "" && $endDate != "" && $transType != "" && $transType == "Settled"){
    ...
    $query .= "SELECT * FROM terminal_reg
    WHERE dateCreated BETWEEN '$startDate' AND '$endDate' AND merchant_id = '$institution_id' AND terminal_status = 'Assigned' AND (settled_balance > '0.0' OR settled_balance > 0) AND (pending_balance = '0.0' OR pending_balance = '0')";
...
{
 $query = "SELECT * FROM terminal_reg WHERE terminal_status = 'Assigned' AND merchant_id = '$institution_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

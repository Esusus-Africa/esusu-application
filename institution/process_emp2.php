...
{		
	...
	$update = mysqli_query($link,"UPDATE user SET name='$name',lname='$lname',mname='$mname',email='$email',phone='$phone',addr1='$addr1',city='$city',state='$state',zip='$zip',country='$country',username='$username', branchid='$branch', role = '$edit_role', cust_reg='$cust_reg', deposit='$deposit', withdrawal='$withdrawal', comment = '$status', dept = '$dept', allow_auth = '$allow_auth' WHERE userid ='$id'") or die("Error: " . mysqli_error($link)); 
	...
	elseif($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif")
	{
		...
		$update = mysqli_query($link,"UPDATE user SET name='$name',lname='$lname',mname='$mname',email='$email',phone='$phone',addr1='$addr1',city='$city',state='$state',zip='$zip',country='$country',comment='$comment',username='$username', image='$location', branchid='$branch', role = '$edit_role', cust_reg='$cust_reg', deposit='$deposit', withdrawal='$withdrawal', comment = '$status', dept = '$dept', allow_auth = '$allow_auth' WHERE userid ='$id'") or die("Error: " . mysqli_error($link)); 
		($pos_wallet_settings != '1' || $walletId == "" ? "" : (($fetchWalletNum == 1 ) ? mysqli_query($link,"UPDATE pos_wallet SET walletid = '$walletId', username = '$wallet_uname', password = '$wallet_pass' WHERE tid = '$posid'") or die ("Error: " . mysqli_error($link)) : mysqli_query($link,"INSERT INTO pos_wallet VALUES(null,'$created_by','$utid','$wallet_uname','$wallet_pass','$walletId',NOW())") or die ("Error: " . mysqli_error($link))));

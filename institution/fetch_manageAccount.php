...
$column = array('id', 'Action', 'Branch', 'A/c Officer', 'A/c Number', 'A/c Name', 'Bank Name', 'Phone Number', 'Email Address', 'Balance', 'Status', 'Date/Time');
...
foreach($result as $row)
{
 ...
 $sub_array[] = "<div class='btn-group'>
                    ...
                    ".(($view_bank_account_info == '1') ? "<li><p><a href='view_acctInfo.php?id=".$_SESSION['tid']."&&idm=".$row['id']."&&mid=OTIy' class='btn btn-default btn-flat'><i class='fa fa-eye'></i> Account Info </a></p></li>" : '<span style="color: red;">--Not Authorized--</span>')."
                    ".(($transfer_fund_from_bank_account == '1') ? "<li><p><a href='dotransfer.php?id=".$_SESSION['tid']."&&idm=".$row['id']."&&mid=OTIy' class='btn btn-default btn-flat'><i class='fa fa-money'></i> Transfer Fund </a></p></li>" : '<span style="color: red;">--Not Authorized--</span>')."
                    </ul>
                    </div>
                </div>";
 ...
 $sub_array[] = $row['currency_code'].number_format($row['balance'],2,".",",");
...
{
 $query = "SELECT * FROM bank_account WHERE merchantid = '$institution_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

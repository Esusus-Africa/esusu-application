...
?>
	<?php echo ($settle_fund == '1') ? '<a href="settle_fund.php?id='.$_SESSION['tid'].'&&idm='.$_GET['idm'].'&&mid=NTEw" class="btn btn-flat bg-'.$myaltrow['alternate_color'].' == "" ? orange : '.$myaltrow['alternate_color'].'" target="_blank"><i class="fa fa-minus"></i>&nbsp;Settle Fund</a>' : ''; ?>		
	<hr>		
...
$tid = $_SESSION['tid'];
$idm = $_GET['idm'];
$select = mysqli_query($link, "SELECT * FROM fund_settlement WHERE companyid = '$institution_id' ORDER BY id DESC") or die (mysqli_error($link));
...
$tid = $_SESSION['tid'];
$idm = $_GET['idm'];
$select = mysqli_query($link, "SELECT * FROM fund_settlement WHERE companyid = '$institution_id' AND cashier = '$idm' ORDER BY id DESC") or die (mysqli_error($link));
if(mysqli_num_rows($select)==0)
{
echo "<div class='alert bg-blue'>No data found yet!.....Check back later!!</div>";
}

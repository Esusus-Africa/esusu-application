...
<?php echo "<a href='addloanprd.php?id=".$_SESSION['tid']."&&mid=NDEx'><button type='button' class='btn btn-flat bg-".(($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color'])."'><i class='fa fa-plus'></i>&nbsp;Setup Loan Product</button></a>"; ?>
	<hr>		
...
<?php
$date_now = date("Y-m-d");
$select = mysqli_query($link, "SELECT * FROM loan_product WHERE merchantid = '$institution_id' ORDER BY id DESC") or die (mysqli_error($link));

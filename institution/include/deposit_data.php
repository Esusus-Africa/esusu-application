<div class="box">
	       <div class="box-body">
            ...
            ?>
            <h3 class="panel-title"><a href="view_savings.php?id=<?php echo $_SESSION['tid']; ?>&&idm=<?php echo $iuid; ?>&&mid=<?php echo base64_encode("510"); ?>"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button></a><i class="fa fa-money"></i> Customer Deposit Form | <b>Till Balance: -<?php echo number_format($balance,2,'.',','); ?></b>
...
{    
	...
	if($balance < $amount){
		...
		echo "<div class='alert bg-orange'>Oh-Sorry! You have insufficient fund in your Till Account!!</div>";
	...
	elseif($maintenance_row == 1 && $billing_type != "PAYGException" && $iassigned_walletbal < $t_perc){
		...
		echo '<meta http-equiv="refresh" content="1;url=deposit.php?id='.$_SESSION['tid'].'&&mid=NDEw&&pin">';
...
{    
	...
	if($otpnum == 0){
		...
		$totalUnsettledBal = $unsettledBal + $amount;
		...
		if($maintenance_row == 1 && $billing_type != "PAYGException" && $iassigned_walletbal >= $t_perc){
			...
			mysqli_query($link, "UPDATE till_account SET balance = '$remain_balance', unsettled_balance = '$totalUnsettledBal', commission_balance = '$total_commission_bal' WHERE cashier = '$iuid'");

			mysqli_query($link, "INSERT INTO fund_allocation_history VALUES(null,'$txid','$institution_id','$iuid','$isbranchid','$fullname','$iuid','$amount','Debit','$transactionType','$icurrency','$remain_balance','$remark','successful','$date_time')");
			...
			echo '<meta http-equiv="refresh" content="2;url=deposit.php?id='.$_SESSION['tid'].'&&mid=NDEw">';

...
?>
            ...
            <ul class="nav nav-tabs">
              <li <?php echo ($_GET['tab'] == 'tab_1') ? "class='active'" : ''; ?>><a href="purchaseloans.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $lide; ?>&&mid=NDA1&&tab=tab_1">Loan Information</a></li>
              <li <?php echo ($_GET['tab'] == 'tab_2') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'purchaseloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_2'; ?>">Add Product</a></li>
              <li <?php echo ($_GET['tab'] == 'tab_3') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'purchaseloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_3'; ?>">Add Guarantor</a></li>
			  <li <?php echo ($_GET['tab'] == 'tab_3a') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'purchaseloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_3a'; ?>">Upload Document</a></li>
              <li <?php echo ($_GET['tab'] == 'tab_4') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'purchaseloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_4'; ?>">Confirmation</a></li>
            </ul>
...
if(isset($_POST['save_loan']))
{
	...
	*/
      if($iassigned_walletbal < $loan_booking && mysqli_num_rows($isearch_maintenance_model) == 1){  // && $billing_type != "PAYGException"
        ...
        $insert = mysqli_query($link, "INSERT INTO loan_info VALUES(null,'$lid','$lproduct','Purchase','$borrower','$baccount','$income_amt','$salary_date','$employer','$bacinfo','$amount','','','$agent','$unumber','Pending','','','$interest','$amount_topay','$amount_topay','','$lreasons','$upstatus','$p_status','$institution_id','','$idept','$isbranchid','','','','','','','Pending','NotSent','','$wallet_date_time')") or die ("Error: " . mysqli_error($link));
		...
		echo '<meta http-equiv="refresh" content="5;url=purchaseloans.php?id='.$_SESSION['tid'].'&&lide='.$lid.'&&mid=NDA1&&&&tab=tab_2">';
...
?>
			...
			<div class="form-group">
                        ...
                        <select name="lproduct" class="select2" id="loan_products" style="width: 100%;" required>
                              ...
                              <?php
                              $getin = mysqli_query($link, "SELECT * FROM loan_product WHERE (merchantid = '$institution_id' AND category = 'Purchase') OR (merchantid != '$institution_id' AND category = 'Purchase' AND visibility = 'Yes' AND authorize = '1') ORDER BY id") or die (mysqli_error($link));
			 ...
			 <div class="form-group">
				...
				<?php
				$account = $fetchLoan['baccount'];
				$selectB = mysqli_query($link, "SELECT * FROM borrowers WHERE account = '$account'");
				$fetchB = mysqli_fetch_array($selectB);
				?>
				<option value="<?php echo ($lide == "") ? '' : $fetchB['id']; ?>" selected="selected"><?php echo ($lide == "") ? '--Select Customer Account--' : (($fetchB['snum'] == "") ? "" : $fetchB['snum'].' - ').$fetchB['fname'].' '.$fetchB['lname']; ?></option>
				<?php
			...
			<div class="form-group">
				  ...
				  <option value="<?php echo ($lide == "") ? '' : $account; ?>" selected="selected"><?php echo ($lide == "") ? 'Account Loading...' : $account; ?></option>
                <?php
				$getin = mysqli_query($link, "SELECT * FROM borrowers WHERE branchid = '$institution_id' ORDER BY id") or die (mysqli_error($link));
				while($row = mysqli_fetch_array($getin))
				{
...
?>
				...
				<div class="box-footer">
					<button name="addProduct" type="submit" class="btn bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="fa fa-plus"> Add Cart</i></button>
                              <a href="purchaseloans.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $lide; ?>&&mid=NDA1&&tab=tab_3"><button name="Next" type="button" class="btn bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-save"> Proceed</i></button></a> 
				</div>
			</div>
...
?>    
                  ...
                  <?php
                    ...
                    if(mysqli_num_rows($search_file) == 0){
                    	...
                    	while($get_file = mysqli_fetch_array($search_file)){
                    		$i++;
                    ?>
                    <a href="<?php echo $fetchsys_config['file_baseurl'].$get_file['item_path']; ?>" target="_blank"><img src="<?php echo $fetchsys_config['file_baseurl']; ?>file_attached.png" width="20" height="20"><i class="fa fa-eye"></i> Picture<?php echo $i; ?></a>
                    <?php
                    	}
                    }
                  ?>
    			...
    			<td><a href="purchaseloans.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $lide; ?>&&myidm=<?php echo $id; ?>&&mid=NDA1&&tab=tab_2" class="btn bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="fa fa-times"></i> Delete</a></td>
                </tr>
<?php } } ?>
             </tbody>
                </table>
...
if(isset($_GET['myidm'])){
    ...
    echo '<script>window.location="purchaseloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_2"; </script>';
...
?>
			...
			<div align="right">
				<div class="box-footer">
                    ...
                    <a href="purchaseloans.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $lide; ?>&&tab=tab_3a"><button name="Next" type="button" class="btn bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-save"> Proceed</i></button></a> 
				</div>
		      </div>
					...
					<span id='loan_required_doc'>Loading...</span>
						...
						if(isset($_POST['approveDoc'])){
							...
							if($id == ''){
								...
								echo "<script>window.location='purchaseloans.php?id=".$_SESSION['tid']."&&lide=".$lide."&&mid=NDA1&&tab=tab_3a'; </script>";
							}
							else{
								for($i=0; $i < $N; $i++)
								{
...
if(isset($_POST['Yes'])){
    ...
    echo '<meta http-equiv="refresh" content="5;url=purchaseloans.php?id='.$_SESSION['tid'].'&&mid=NDA1&&tab=tab_1">';
...
if(isset($_POST['No'])){
      ...
      mysqli_query($link, "DELETE FROM outgoing_stock WHERE lid = '$lid'");

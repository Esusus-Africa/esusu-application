...
<form method="post">
	...
	<hr>				  
			 <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
<?php
($irole == "agent_manager" || $irole == "institution_super_admin" || $irole == "merchant_super_admin") ? $select = mysqli_query($link, "SELECT * FROM terminal_reg WHERE merchant_id = '$institution_id' AND terminal_status = 'Booked' ORDER BY id DESC") or die (mysqli_error($link)) : "";
($irole != "agent_manager" && $irole != "institution_super_admin" && $irole != "merchant_super_admin") ? $select = mysqli_query($link, "SELECT * FROM terminal_reg WHERE merchant_id = '$institution_id' AND initiatedBy = '$iuid' AND terminal_status = 'Booked' ORDER BY id DESC") or die (mysqli_error($link)) : "";

...
<a href="internalReview.php?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("405"); ?>&&tab=tab_1"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="fa fa-refresh"></i>&nbsp;Refresh</button></a>
...
<div class="col-md-14">
            ...
            <ul class="nav nav-tabs">
             <li <?php echo ($_GET['tab'] == 'tab_1') ? "class='active'" : ''; ?>><a href="internalReview.php?id=<?php echo $_SESSION['tid']; ?>&&mid=NDA1&&tab=tab_1">All Loan Application</a></li>

             <li <?php echo ($_GET['tab'] == 'tab_2') ? "class='active'" : ''; ?>><a href="#internalReview.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $_GET['lide']; ?>&&mid=NDA1&&tab=tab_2">Action</a></li>
...
while($fetch_loanp = mysqli_fetch_array($search_loanp))
{
	...
	($individual_loan_records === "" && $branch_loan_records === "") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND status = 'Internal-Review' ORDER BY id DESC") or die (mysqli_error($link)) : "";
	($individual_loan_records === "1") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND (agent = '$iname' OR agent = '$iuid') AND status = 'Internal-Review' ORDER BY id DESC") or die (mysqli_error($link)) : "";
	($branch_loan_records === "1") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND sbranchid = '$isbranchid' AND status = 'Internal-Review' ORDER BY id DESC") or die (mysqli_error($link)) : "";
...
?> 
			...
			<?php echo ($approve_disapprove_loans == '1') ? "<a href='internalReview.php?id=".$_SESSION['tid']."&&lide=".$id."&&mid=NDA1&&tab=tab_2'> <button type='button' class='btn bg-".(($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color'])."'><i class='fa fa-edit'></i></button></a>" : "--"; ?>
<?php
...
{
	$lp_id = $fetch_loanp['id'];*/
	($individual_loan_records === "" && $branch_loan_records === "") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND status = 'Internal-Review' AND dept = '$idept' ORDER BY id DESC") or die (mysqli_error($link)) : "";
	($individual_loan_records === "1") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND (agent = '$iname' OR agent = '$iuid') AND status = 'Internal-Review' AND dept = '$idept' ORDER BY id DESC") or die (mysqli_error($link)) : "";
	($branch_loan_records === "1") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND sbranchid = '$isbranchid' AND status = 'Internal-Review' AND dept = '$idept' ORDER BY id DESC") or die (mysqli_error($link)) : "";
	while($row = mysqli_fetch_array($select))
	{
	$id = $row['id'];
	$lid = $row['lid'];
								...
								<button type="submit" name="update_status" class="btn bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="icon-forward"></i>&nbsp;Process</button>
    								...
    								<?php
									    ...
									    if($Status_save == "Internal-Review"){
											...
											(strlen($phone) == "14" || strlen($phone) == "13" || strlen($phone) == "11") ? mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$refid','$phone','$sms_rate','NGN','system','SMS Content: $message','successful',NOW())") : "";
									        ...
									        echo "<script>window.location='internalReview.php?id=".$_SESSION['tid']."&&mid=NDA1&&tab=tab_1'; </script>";
									    }

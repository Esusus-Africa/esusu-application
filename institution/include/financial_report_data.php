<div class="box">
	       <div class="box-body">
			...
			<form class="form-horizontal" method="post" enctype="multipart/form-data">
			 <div class="alert bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"> The Financial Report shows the <b>Income and Expenses in tabular form called Profit and Loss Account.</b>. </div>
             <div class="box-body">
			...
			<div class="form-group">
                  <label for="" class="col-sm-2 control-label" style="color:<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>;">From</label>
                  <div class="col-sm-4">
                  <input name="dfrom" type="date" class="form-control" placeholder="To Date: 2018-05-01">
				  <span style="color: <?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">Date should be in this format: 2018-05-01</span>
                  </div>
				  
				  <label for="" class="col-sm-2 control-label" style="color:<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>;">To</label>
                  <div class="col-sm-4">
                  <input name="dto" type="date" class="form-control" placeholder="To Date: 2018-05-24">
				  <span style="color: <?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">Date should be in this format: 2018-05-24</span>
                  </div>
             </div>
...
$select = mysqli_query($link, "SELECT SUM(amount_paid) FROM saas_subscription_trans WHERE duration_from BETWEEN '$dfrom' AND '$dto' AND status = 'Paid' AND coopid_instid = '$institution_id' ORDER BY id");
while($row = mysqli_fetch_array($select))
{
	...
	$select1 = mysqli_query($link, "SELECT SUM(amount) FROM all_savingssub_transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND status = 'success' AND merchant_id = '$institution_id' ORDER BY id");
	while($fetch1 = mysqli_fetch_array($select1))
	{
		//TOTAL SAVINGS INCOME
		$sum1 = $fetch1['SUM(amount)'];

		$select2 = mysqli_query($link, "SELECT SUM(amount_to_pay) FROM payments WHERE pay_date BETWEEN '$dfrom' AND '$dto' AND remarks = 'paid' AND branchid = '$institution_id' ORDER BY id");
		while($fetch2 = mysqli_fetch_array($select2))
		{
			//TOTAL LOAN REPAYMENT
			$sum2 = $fetch2['SUM(amount_to_pay)'];

			$select3 = mysqli_query($link, "SELECT SUM(amount) FROM transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND t_type = 'Withdraw-Charges' AND branchid = '$institution_id'");
			while($fetch3 = mysqli_fetch_array($select3))
			{
				//TOTAL WITHDRAWAL CHARGES
				$sum3 = $fetch3['SUM(amount)'];

				$select11 = mysqli_query($link, "SELECT SUM(icm_amt) FROM income WHERE icm_date BETWEEN '$dfrom' AND '$dto' AND companyid = '$institution_id'");
				while($fetch11 = mysqli_fetch_array($select11))
				{
					//OTHER INCOME IN TOTAL
					$sum11 = $fetch11['SUM(icm_amt)'];

					$select4 = mysqli_query($link, "SELECT SUM(amount) FROM transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND t_type = 'Deposit' AND branchid = '$institution_id'");
					while($fetch4 = mysqli_fetch_array($select4))
					{
						...
						///////////////////////////////ALL EXPENSES/////////////////////////////////
						$select5 = mysqli_query($link, "SELECT SUM(amount) FROM transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND t_type = 'Withdraw' AND branchid = '$institution_id'");
						while($fetch5 = mysqli_fetch_array($select5))
						{
							//TOTAL AMOUNT WITHDRAWN
							$sum5 = $fetch5['SUM(amount)'];

							$select6 = mysqli_query($link, "SELECT SUM(paid_amount) FROM payroll WHERE pay_date BETWEEN '$dfrom' AND '$dto' AND branchid = '$institution_id'");
							while($fetch6 = mysqli_fetch_array($select6))
							{
								//TOTAL EMPLOYEE SALARY PAID
								$sum6 = $fetch6['SUM(paid_amount)'];

								$select7 = mysqli_query($link, "SELECT SUM(amount) FROM referral_incomehistory WHERE tdate BETWEEN '$dfrom' AND '$dto' AND referral_id = '$institution_id'");
								while($fetch7 = mysqli_fetch_array($select7))
								{
									//TOTAL REFERRAL BONUS PAID
									$sum7 = $fetch7['SUM(amount)'];

									$select8 = mysqli_query($link, "SELECT SUM(amount) FROM loan_info WHERE date_release BETWEEN '$dfrom' AND '$dto' AND status = 'Approved' AND branchid = '$institution_id'");
									while($fetch8 = mysqli_fetch_array($select8))
									{
										//TOTAL LOAN RELEASED
										$sum8 = $fetch8['SUM(amount)'];

										$select9 = mysqli_query($link, "SELECT SUM(eamt) FROM expenses WHERE edate BETWEEN '$dfrom' AND '$dto' AND branchid = '$institution_id'");
...
?>
               ...
               <form method="post">
                	<input type='button' id='btnprint' class='btn bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>' value='Print'>
					<input type="submit" name="generate_pdf" class="btn bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>" value="Generate PDF"/>
				</form>
...
{
	echo "<script>window.open('../pdf/view/pdf_financial1.php?dfrom=".$dfrom."&&dto=".$dto."&&id=".$institution_id."', '_blank'); </script>";
}
?>

...
<a href="activeloans.php?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("405"); ?>"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="fa fa-refresh"></i>&nbsp;Refresh</button></a>
...
while($fetch_loanp = mysqli_fetch_array($search_loanp))
{
	...
	($view_all_loans === "1" && $individual_loan_records === "" && $branch_loan_records === "") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND p_status != 'PAID' AND (status = 'Disbursed' OR status = 'Approved') ORDER BY id DESC") or die (mysqli_error($link)) : "";
	($view_all_loans === "" && $individual_loan_records === "1" && $branch_loan_records === "") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND (agent = '$iname' OR agent = '$iuid') AND p_status != 'PAID' AND (status = 'Disbursed' OR status = 'Approved') ORDER BY id DESC") or die (mysqli_error($link)) : "";
	($view_all_loans === "" && $individual_loan_records === "" && $branch_loan_records === "1") ? $select = mysqli_query($link, "SELECT * FROM loan_info WHERE branchid = '$institution_id' AND sbranchid = '$isbranchid' AND p_status != 'PAID' AND (status = 'Disbursed' OR status = 'Approved') ORDER BY id DESC") or die (mysqli_error($link)) : "";
...
<?php
						if(isset($_POST['delete'])){
    						...
    						if($id == ''){
        						...
        						echo "<script>window.location='activeloans.php?id=".$_SESSION['tid']."&&mid=NDA1&&tab=tab_1'; </script>";
        					}
    						else{
    							for($i=0; $i < $N; $i++)
    							{

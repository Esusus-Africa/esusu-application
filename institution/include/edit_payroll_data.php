<div class="box">
	       <div class="box-body">
            ...
            <h3 class="panel-title"> <a href="listpayroll.php?id=<?php echo $_SESSION['tid']; ?>&&mid=NDIz"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button> </a> <i class="fa fa-paypal"></i>  Edit Payroll</h3>
            </div>
...
if(isset($_POST['save']))
{
	...
	$url = $protocol . $_SERVER['HTTP_HOST']."/application/generate_payslip.php?id=".$idm;
	$id = rand(10000,99999);
	$shorturl = base_convert($id,20,36);
	
	$insert = mysqli_query($link, "INSERT INTO short_urls VALUES(id,'$url','$shorturl')") or die ("Error: " . mysqli_error($link));
	
	$shortenedurl = $protocol . $_SERVER['HTTP_HOST'] . '/index.php?key=' . $shorturl;
...
<div class="alert bg-orange fade in"> <b>Net Pay Distribution</b> </div>
			...
			<p><span style="color: <?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;"><b>NOTE: </b></span><span style="color: <?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>;"> Please make sure there is an email address for the staff since a <b>copy of this Payslip</b> will be send to Staff Inbox. If not, please <a href="view_emp.php?id=<?php echo $_GET['staff_id']; ?>&&mid=NDA5" target="_blank">edit the staff</a> and enter email.

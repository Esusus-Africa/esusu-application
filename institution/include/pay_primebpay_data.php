...
if(isset($_POST['PayBill1']))
{
    ...
    if($my_amount <= 0){
    ...
    }elseif($itransfer_balance < $my_amount){
        ...
        $commission = $my_amount * $ibillPaymentCommission;
        $amount = $my_amount - $commission;
        
        $transactionDateTime = date("Y-m-d h:i:s");
        $dataToProcess = $reference."|".$categories."|".$productcode."|".$customerid."|".$cust_name."|".$pcode."|".$my_amount."|".$itransfer_balance;
        //insert txt waiting list
        $mytxtstatus = 'PAPending';
        ($itransfer_balance < $my_amount) ? "" : mysqli_query($link, "INSERT INTO api_txtwaitinglist VALUE(null,'$iuid','$reference','$dataToProcess','$mytxtstatus','$transactionDateTime')");
        
        //Get my wallet balance after debiting
        $senderBalance2 = $itransfer_balance - $amount;
        ($itransfer_balance < $my_amount) ? "" : mysqli_query($link, "UPDATE user SET transfer_balance = '$senderBalance2' WHERE id = '$iuid'");
        ...
        ($itransfer_balance < $my_amount) ? "" : $response = curl_exec($curl);
        ($itransfer_balance < $my_amount) ? "" : $prime_generate = json_decode($response, true);

        if(($prime_generate['status'] == "201" || $prime_generate['status'] == "200") && $itransfer_balance >= $my_amount){
        	...
        	$insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$customerid - $cust_name','','$amount','Debit','$icurrency','$pcode - WEB','$service_msg','successful','$datetime','$iuid','$senderBalance2','')");
        	
        	$insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$customerid - $cust_name','$commission','','Credit','$icurrency','$pcode - WEB','$service_msg','successful','$datetime','$iuid','$senderBalance2','')");

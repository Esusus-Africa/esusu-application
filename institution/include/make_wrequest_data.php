<div class="box">
	       <div class="box-body">
            ...
            <h3 class="panel-title"><i class="fa fa-money"></i> Customer Withdrawal Request Form
...
{   
	...
	$bal = ($balanceToImpact == "ledger" ? $get_details['balance'] : ($balanceToImpact == "target" ? $get_details['target_savings_bal'] : ($balanceToImpact == "investment" ? $get_details['investment_bal'] : ($balanceToImpact == "asset" ? $get_details['asset_acquisition_bal'] : 0))));
	...
	elseif($maintenance_row == 1 && $billing_type != "PAYGException" && $iwallet_balance < $t_perc){
		...
		mysqli_query($link,"INSERT INTO otp_confirmation VALUES(null,'$iuid','$myiepin','$mydata','Pending','$currenctdate')") or die("Error: " . mysqli_error($link));
		...
		echo '<meta http-equiv="refresh" content="5;url=make_wrequest.php?id='.$_SESSION['tid'].'&&mid=NDEw&&pin">';
...
{    
	...
	if($otpnum == 0){
		...
		$totalAmt = $amount + $final_charges;

		$sendSMS->withdrawalRequestNotifier($email, $fullname, $iname, $inst_name, $correctdate, $ptype, $account, $icurrency, $amount, $bal, $remark, $iemailConfigStatus, $ifetch_emailConfig);
		
		($maintenance_row == "1" && $billing_type != "PAYGException" && $t_perc != "0") ? mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$txid','self','','$t_perc','Debit','$icurrency','Charges','Description: Service Charge for Withdrawal of $amount','successful','$date_time','$iuid','$myiwallet_balance','')") : "";
        ...
        ($final_charges != 0 && $final_charges != "") ? mysqli_query($link, "INSERT INTO transaction VALUES(null,'$txid','$transactionType1','$ptype','$account','----','$fn','$ln','$em','$ph','$final_charges','$iuid','Charges','$correctdate','$institution_id','$isbranchid','$icurrency','','$total','$status','$notification','$notification','$checksms','$balanceToImpact')") or die ("Error: " . mysqli_error($link)) : "";
		
		mysqli_query($link, "INSERT INTO transaction VALUES(null,'$txid','$transactionType','$ptype','$account','----','$fn','$ln','$em','$ph','$amount','$iuid','$remark','$correctdate','$institution_id','$isbranchid','$icurrency','','$total','$status','$notification','$notification','$checksms','$balanceToImpact')") or die ("Error: " . mysqli_error($link));
		
		mysqli_query($link, "INSERT INTO ledger_withdrawal_request VALUES(null,'$txid','$institution_id','$iuid','$account','$ptype','$totalAmt','$remark','Pending','$balanceToImpact','$icurrency','$em','$ph','$isbranchid','0','0','$date_time')") or die ("Error: " . mysqli_error($link));
		
		mysqli_query($link, "DELETE FROM otp_confirmation WHERE userid = '$iuid' AND otp_code = '$myotp' AND status = 'Pending'");

		echo "<div align='center'><img src='../image/checkmark.gif'><p style='color: #38A1F3;'>Withdrawal Request Sent Successfully</p></div>";
		echo '<meta http-equiv="refresh" content="5;url=make_wrequest.php?id='.$_SESSION['tid'].'&&mid=NDEw">';

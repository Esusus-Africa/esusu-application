...
if(isset($_POST['PayBill1']))
{
    ...
    if($itransfer_balance < $amount_recharge){
        ...
        //OTP ENABLED
        $update = mysqli_query($link,"INSERT INTO otp_confirmation VALUES(null,'$iuid','$otpcode','$mydata','Pending','$currenctdate')")or die(mysqli_error());
        //SEND SMS
        $sendSMS->userGeneralAlert($ozeki_password, $ozeki_url, $sysabb, $myiphone, $sms, $institution_id, $sms_refid, $sms_rate, $iuid, $imywallet_balance, $debitWallet, $userType);
        ...
        $senderBalance = $itransfer_balance - $amount_recharge;
        ...
        if($result1['ResponseCode'] == "100"){
            ...
            $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$myref','$ph','','$amount_recharge','Debit','$icurrency','Airtime - WEB','Airtime Topup for $operator with refid: $myref via WEB','successful',NOW(),'$iuid','$senderBalance','')");
        	...
        	$update_records = mysqli_query($link, "UPDATE user SET transfer_balance = '$senderBalance' WHERE id = '$iuid'");
		...
		if($jsonData['failed'] == "0")
		{
		    foreach($jsonData['result'] as $newOutput){
		        ...
		        if($newOutput['response'] == "OK"){
	                ...
	                //$remainBalance = $itransfer_balance - $amount;
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','','$amount','Debit','$icurrency','Airtime - WEB','Airtime Topup for $operator with refid: $myref via WEB','successful',NOW(),'$iuid','$senderBalance2','')");
        		    
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','$commission','','Credit','$icurrency','Commission - WEB','Airtime Topup Commission for $operator with refid: $myref via WEB','successful',NOW(),'$iuid','$senderBalance2','')");
        		    
        		    mysqli_query($link, "UPDATE user SET transfer_balance = '$senderBalance2' WHERE id = '$iuid'");
        		    ...
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','','$amount','Debit','$icurrency','Airtime - WEB','Airtime Topup for $operator with refid: $myref via WEB','pending',NOW(),'$iuid','$itransfer_balance','')");
        		    
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','$commission','','Credit','$icurrency','Commission - WEB','Airtime Topup Commission for $operator with refid: $myref via WEB','pending',NOW(),'$iuid','$itransfer_balance','')");
	...
	if($otpnum == 0){
	    ...
	    echo "<div class='alert bg-orange'>Opps!...Invalid Credentials!!</div>";
            ...
            if($result1['ResponseCode'] == "100"){
                ...
                $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$iuid','$myref','$ph','','$amount_recharge','Debit','$icurrency','Airtime - WEB','Airtime Topup for $operator with refid: $myref via WEB','successful','$currenctdate','$iuid','$senderBalance','')");
        ...
        if($jsonData['failed'] == "0")
		{
		    foreach($jsonData['result'] as $newOutput){
		        ...
		        if($newOutput['response'] == "OK"){
        		    ...
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','','$amount','Debit','$icurrency','Airtime - WEB','Airtime Topup for $operator with refid: $myref via WEB','successful',NOW(),'$iuid','$senderBalance','')");
        		    
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','$commission','','Credit','$icurrency','Commission - WEB','Airtime Topup for $operator with refid: $myref via WEB','successful',NOW(),'$iuid','$senderBalance','')");
        		    ...
        		    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$institution_id','$transaction_id','$phone','$commission','','Credit','$icurrency','Commission - WEB','Airtime Topup for $operator with refid: $myref via WEB','pending',NOW(),'$iuid','$itransfer_balance','')");

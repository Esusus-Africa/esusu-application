...
<a href="create_transfer_recipient.php?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("404"); ?>"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="fa fa-plus"></i>&nbsp;Add Transfer Recipient</button></a>
	<hr>		
			 ...
			 <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody> 
<?php
($individual_customer_records != "1" && $branch_customer_records != "1") ? $select = mysqli_query($link, "SELECT * FROM transfer_recipient WHERE companyid = '$institution_id' ORDER BY id") or die (mysqli_error($link)) : "";
($individual_customer_records === "1" && $branch_customer_records != "1") ? $select = mysqli_query($link, "SELECT * FROM transfer_recipient WHERE companyid = '$institution_id' AND staffid = '$iuid' ORDER BY id") or die (mysqli_error($link)) : "";
($individual_customer_records != "1" && $branch_customer_records === "1") ? $select = mysqli_query($link, "SELECT * FROM transfer_recipient WHERE companyid = '$institution_id' AND sbranchid = '$isbranchid' ORDER BY id") or die (mysqli_error($link)) : "";

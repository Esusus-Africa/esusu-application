<div class="box">
	       <div class="box-body">
			...
			<form class="form-horizontal" method="post" enctype="multipart/form-data">
			 <div class="alert bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"> The Savings Transactions Report (Recurring) shows the <b>Transaction charges on different subscribed savings plan with their respective status.</b>. </div>
             <div class="box-body">
...
$status = $_GET['status'];
$acctno = $_GET['acctno'];
$select = mysqli_query($link, "SELECT * FROM all_savingssub_transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND status = '$status' AND acn = '$acctno' OR (date_time BETWEEN '$dfrom' AND '$dto' AND status = '$status' AND merchant_id = '$institution_id') ORDER BY id") or die (mysqli_error($link));
...
<?php } ?>
               ...
               <form method="post">
                	...
                	<a href="excel_rsavings1.php?dfrom=<?php echo $dfrom; ?>&&dto=<?php echo $dto; ?>&&status=<?php echo $status; ?>&&acctno=<?php echo $acctno; ?>&&id=<?php echo $institution_id; ?>" class="btn bg-orange" target="_blank"><i class="fa fa-export"></i> Export to Excel</a>
...
{
	echo "<script>window.open('../pdf/view/pdf_rsavings1.php?dfrom=".$dfrom."&&dto=".$dto."&&status=".$status."&&acctno=".$acctno."&&id=".$institution_id."', '_blank'); </script>";
}
?>

...
if(isset($_POST['reqWith']))
{
    ...
    $DateTime = date('Y-m-d').' '.(date(h) + 1).':'.date('i:s');

    $searchTerm = mysqli_query($link, "SELECT * FROM terminal_reg WHERE terminal_id = '$terminal' AND trace_id = '$trace_id'");
    $fetchTerm = mysqli_fetch_array($searchTerm);
    $channel = $fetchTerm['channel'];
    $traceid = $fetchTerm['trace_id'];
    ...
    elseif($itransfer_balance < $myCgate_charges){
        ...
        $search_restapi1 = mysqli_query($link, "SELECT * FROM restful_apisetup WHERE api_name = 'gtbcgate_invokereference'");
        $fetch_restapi1 = mysqli_fetch_object($search_restapi1);
        $api_url1 = $fetch_restapi1->api_url;
            
        $postdata = '{"userInfo": {"userName":"'.$gtbcgate_username.'","password": "'.$gtbcgate_password.'"},' . 
                    '"posRequest": {"terminalId":"'.$terminal.'","Channel":"'.$channel.'","Amount":'.$amt.',' . 
        ...
        if($responseCode == "00"){
            ...
            $insert = mysqli_query($link, "UPDATE terminal_reg SET pending_balance = '$pending_balance' WHERE merchant_id = '$institution_id' AND terminal_id = '$terminal' AND trace_id = '$trace_id'");
            ($phone == "" || $iassigned_walletbal < $sms_charges) ? "" : mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$TransactionID','$phone','','$sms_charges','Debit','NGN','Charges','SMS Content: $sms','successful','$currentDate','$iuid','$mywallet_balance','')");
...
if(isset($_POST['requery'])){
    ...
    $search_restapi1 = mysqli_query($link, "SELECT * FROM restful_apisetup WHERE api_name = 'gtbcgate_requery'");
    $fetch_restapi1 = mysqli_fetch_object($search_restapi1);
    $api_url1 = $fetch_restapi1->api_url;

    $postdata = '{"RequestHeader": {"Username":"'.$gtbcgate_username.'","Password": "'.$gtbcgate_password.'"},' . 
    ...
    elseif($responseCode == "09" && $tStatus == "Pending"){ //PENDING STATUS
        ...
        $pBal = $fetchTerminal['pending_balance'] - $amount;
...
?>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    <?php
                    $search = mysqli_query($link, "SELECT * FROM terminal_reg WHERE terminal_status = 'Assigned' AND merchant_id = '$institution_id' AND tidoperator = '$iuid' AND channel = 'USSD'");
                    $get_search = mysqli_fetch_array($search);
                    ?>
                    <strong style="font-size:20px;"><?php echo ($get_search['terminal_id'] == "") ? "No Terminal Assigned!" : $get_search['terminal_id']; ?></strong> <?php echo (isset($_GET['txid'])) ? '<button name="requery" type="submit" class="btn bg-'.(($myaltrow['theme_color'] == "") ? "blue" : $myaltrow['theme_color']).'"><i class="fa fa-refresh">&nbsp;Check Status</i></button>' : ''; ?>
                    <input name="terminal" value="<?php echo $get_search['terminal_id']; ?>" type="hidden"/>
                    <input name="trace_id" value="<?php echo $get_search['trace_id']; ?>" type="hidden"/>
                </div>
            ...
            <div class="form-group">
                <label for="" class="col-sm-3 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">Customer Phone (Optional):</label>
                <div class="col-sm-6">
            ...
            <div class="form-group">
                <label for="" class="col-sm-3 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">Customer Email (Optional):</label>
                <div class="col-sm-6">

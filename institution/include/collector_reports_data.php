<div class="box">
	       <div class="box-body">
			...
			<form class="form-horizontal" method="post" enctype="multipart/form-data">
			 <div class="alert bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"> The Collectors report shows the <b>total capital, total interest, and total collection</b>. </div>
             <div class="box-body">
			...
			<div class="form-group">
             ...
             <div class="form-group">
                  ...
                  <div class="col-sm-4">
                  <select name="branchid"  class="form-control select2" style="width:100%">
					<option selected="selected">Filter By Branch...</option>
					<?php
					$get = mysqli_query($link, "SELECT * FROM branches WHERE branchid = '$institution_id' ORDER BY id") or die (mysqli_error($link));
				  ...
				  <label for="" class="col-sm-2 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">Staff</label>
                  <div class="col-sm-4">
                  <select name="staffid"  class="form-control select2" style="width:100%">
					<option selected="selected">Filter By Staff...</option>
					<?php
					$get = mysqli_query($link, "SELECT * FROM user WHERE created_by = '$institution_id' ORDER BY id") or die (mysqli_error($link));
					while($rows = mysqli_fetch_array($get))
					{
					?>
...
if(isset($_POST['search'])){
	...
	echo "<script>window.location='collector_reports.php?id=".$_SESSION['tid']."&&dfrom=".$dfrom."&&dto=".$dto."&&branchid=".$bid."&&staffid=".$sid."&&mid=NDI1'; </script>";
}
?>
...
$select = mysqli_query($link, "SELECT borrower, sum(amount), agent, sum(interest_rate) FROM loan_info WHERE date_release BETWEEN '$dfrom' AND '$dto' OR branchid = '$branchid' OR teller = '$staffname' ORDER BY id") or die (mysqli_error($link));

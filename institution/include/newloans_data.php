...
?>
            ...
            <ul class="nav nav-tabs">
              <li <?php echo ($_GET['tab'] == 'tab_1') ? "class='active'" : ''; ?>><a href="newloans.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $lide; ?>&&mid=NDA1&&tab=tab_1">Loan Information</a></li>
              <li <?php echo ($_GET['tab'] == 'tab_2') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'newloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_2'; ?>">Add Guarantor</a></li>
			  <li <?php echo ($_GET['tab'] == 'tab_2a') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'newloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_2a'; ?>">Upload Document</a></li>
              <li <?php echo ($_GET['tab'] == 'tab_3') ? "class='active'" : ''; ?>><a href="<?php echo ($lide == "") ? '#' : 'newloans.php?id='.$_SESSION['tid'].'&&lide='.$lide.'&&mid=NDA1&&tab=tab_3'; ?>">Confirmation</a></li>
            </ul>
...
if(isset($_POST['save_loan']))
{
	...
	*/
	if($iassigned_walletbal < $loan_booking && mysqli_num_rows($isearch_maintenance_model) == 1){ // && $billing_type != "PAYGException"
		...
		mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$refid','self','','$loan_booking','Debit','NGN','Charges','Description: Maintenance fee for Loan Booking','successful','$wallet_date_time','$iuid','$myiwallet_balance','')");
		...
		echo '<meta http-equiv="refresh" content="5;url=newloans.php?id='.$_SESSION['tid'].'&&lide='.$lid.'&&mid=NDA1&&&&tab=tab_2">';
...
?>
			...
			<div class="form-group">
				  ...
				  <option value="<?php echo ($lide == "") ? '' : $account; ?>" selected="selected"><?php echo ($lide == "") ? 'Loading Account...' : $account; ?></option>
                <?php
		...
		?>
		 ...
		 <div class="form-group">
                      ...
                      <div class="col-sm-10">
            <select name="country"  class="form-control select2" id="country" onchange="loadbank();">
			...
			<div class="form-group">
                  ...
                  <div class="col-sm-10">
                  <input name="acct_no" type="text" id="account_number" onkeydown="loadaccount();" class="form-control" placeholder="Account Number">
	 ...
	 <div class="form-group">
				...
				<span style="color: <?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>;"><b>TELL US WHY THE CUSTOMER NEED LOAN!!</b></span>
          			 </div>
				 </div>
...
?>
				...
				<div align="right">
					<div class="box-footer">
                        ...
                        <a href="newloans.php?id=<?php echo $_SESSION['tid']; ?>&&lide=<?php echo $lide; ?>&&mid=NDA1&&&tab=tab_2a"><button name="Next" type="button" class="btn bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-save"> Proceed</i></button></a> 
					</div>
				</div>
					...
					<span id='loan_required_doc'>Loading...</span>
						...
						if(isset($_POST['approveDoc'])){
							...
							if($id == ''){
								...
								echo "<script>window.location='newloans.php?id=".$_SESSION['tid']."&&lide=".$lide."&&mid=NDA1&&&tab=tab_2a'; </script>";
							}
							else{
								for($i=0; $i < $N; $i++)
								{

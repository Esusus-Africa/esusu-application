...
if (isset($_POST['save'])) 
{
    ...
    if(!(filter_var($pospin, FILTER_VALIDATE_INT))){

        echo'<span class="itext" style="color: blue;">Invalid POS Pin Format!!</span>';

    }
    elseif(strlen($pospin) != "6"){

        echo'<span class="itext" style="color: blue;">POS Pin Length must be 6 digit number!!</span>';

    }
    elseif($image == "")
	{
		mysqli_query($link,"UPDATE user SET name='$fname',lname='$lname',mname='$mname',email='$email',phone='$number',addr1='$add1',city='$city',state='$state',zip='$zip',username='$user',password='$decript',tpin='$tpin',dob='$dob',gender='$gender',pospin='$pospin' WHERE id ='$id'")or die(mysqli_error()); 
		...
		elseif($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif")
		{
			...
			mysqli_query($link,"UPDATE user SET name='$fname',lname='$lname',mname='$mname',email='$email',phone='$number',addr1='$add1',city='$city',state='$state',zip='$zip',username='$user',password='$decript',image='$location',tpin='$tpin',dob='$dob',gender='$gender',pospin='$pospin' WHERE id ='$id'")or die(mysqli_error()); 
...
<hr>	
				 ...
				 <div class="form-group">
                  ...
                  <div class="form-group">
                  <label for="" class="col-sm-2 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">POS Pin</label>
                  <div class="col-sm-10">
                  <input name="pospin" type="password" class="form-control" value="<?php echo $row['pospin'];?>" maxlength="6" id="password-field" required>
...
<?php
  ...
  ?>
             ...
             <span id="success_message"></span>
                ...
                <div class="form-group">
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
    <?php
    $selectAttachment = mysqli_query($link, "SELECT * FROM attachment WHERE borrowerid = '$iuid' ORDER BY id DESC") or die (mysqli_error($link));
    ...
    <?php } } ?>
                  ...
                  if(isset($_POST['verifyBVN'])){
                       ...
                       elseif($ibvn_route == "Wallet Africa"){
                            ...
                            if($ResponseCode == "200"){
                                ...
                                $search_bvnverify = mysqli_query($link, "SELECT * FROM bvn_log WHERE accountID = '$iuid'");
								...
								($bvn_nos == 1) ? $update_bvn = mysqli_query($link, "UPDATE bvn_log SET mydata = '$mybvn_data' WHERE accountID = '$iuid'") : $insert_bvn = mysqli_query($link, "INSERT INTO bvn_log VALUE(null,'$institution_id','$isbranchid','$iuid','$iAcctOfficer','$mybvn_data','$ibvn_fee','$wallet_date_time','$rOrderID')");
                                ...
                                ($bvn_dob == $default_dob || $bvn_phone == $myiphone) ? mysqli_query($link, "UPDATE user SET addr2 = '$userBvn' WHERE id = '$iuid'") : "";
                                   
                                ($bvn_dob == $default_dob || $bvn_phone == $myiphone) ? mysqli_query($link, "UPDATE virtual_account SET acct_status = 'UnderReview' WHERE userid = '$iuid'") : "";
                ...
                <?php
                $lookup_bvn = mysqli_query($link, "SELECT * FROM bvn_log WHERE accountID = '$iuid'");
...
if(isset($_POST['upload']))
{
    ...
    $search_Otp = mysqli_query($link, "SELECT * FROM bvn_log WHERE accountID = '$iuid'");
    if(mysqli_num_rows($search_Otp) == 1)
    {
        ...
        ($mybvn == "") ? "" : mysqli_query($link, "UPDATE virtual_account SET acct_status = 'UnderReview' WHERE userid = '$iuid'");
        mysqli_query($link, "UPDATE user SET name = '$confirmed_fname', lname = '$confirmed_lname', mname = '$confirmed_mname', dob = '$confirmed_dob', addr2 = '$mybvn' WHERE id = '$iuid'");
        ...
        mysqli_query($link, "UPDATE user SET addr2 = '$mybvn' WHERE id = '$iuid'");

...
if(isset($_POST['FundWallet']) && ($fetchsys_config['mo_status'] == "NotActive"))
{
	...
	$redirect_url = 'https://esusu.app/institution/mywallet.php?id='.$_SESSION['tid'].'&&o_amt='.$secure_amount.'&&refid='.$reference.'&&mid=NDA3';
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_POSTFIELDS => json_encode([
        'amount'=>$cal_charges,
        'customerName'=>$inst_name,
        'customerEmail'=>$inst_email,
        'paymentReference'=>$reference,
        'paymentDescription'=>'Fund Wallet: '.$institution_id,
    ...
    if ($err) {
      echo "cURL Error #:" . $err;
        ...
        $walletBal = $itransfer_balance + $amount;
        
        //mysqli_query($link, "UPDATE merchant_reg SET wallet_balance = '$mybalance' WHERE merchantID = '$company_id'");
        mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$reference','self','$amount','','Credit','$icurrency','MNFY','','pending','$datetime','$iuid','$walletBal','')");
        // uncomment out this line if you want to redirect the user to the payment page

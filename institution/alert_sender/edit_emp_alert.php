...
if($iassigned_walletbal >= $sms_rate)
{
    ...
    $search_cust = mysqli_query($link, "SELECT * FROM user WHERE userid ='$id'") or die("Error:" . mysqli_error($link));
    $get_cust = mysqli_fetch_array($search_cust);
    $phone = $get_cust['phone'];
    
    $message = "$sender>>>Profile Update | ";
    $message .= "Your profile has been updated successfully ";
    $message .= "If you are not aware or responsible of/for this changes, kindly contact us.";
    $debug = true;
    ozekiSend($sender,$phone,$message,$debug);
    mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$refid','$phone','','$sms_rate','Debit','NGN','system','SMS Content: $message','successful','$date_time','$iuid','$mybalance','')");

...
if($iassigned_walletbal >= $sms_rate)
{
    ...
    $search_cust = mysqli_query($link, "SELECT * FROM borrowers WHERE card_id = '$card_id'") or die("Error:" . mysqli_error($link));
    $get_cust = mysqli_fetch_array($search_cust);
    $phone = $get_cust['phone'];
    
    $search_wcod = mysqli_query($link, "SELECT * FROM card_withdrawal_confirmation WHERE otp_code = '$otp_code'") or die("Error:" . mysqli_error($link));
    $get_wcod = mysqli_fetch_array($search_wcod);
    $amount = number_format($get_wcod['amount'],2,'.',',');
    $withcurrency = $get_wcod['currency'];
    
    $message = "$sysabb>>>CARD WITHDRAWAL OTP.";
    $message .= " Your One Time Password to Transfer ".$withcurrency.$amount." is ".$otp_code.". Kindly ignore if you are unaware. Thanks.";
    $debug = true;

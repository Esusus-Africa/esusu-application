...
<script src="https://api.ravepay.co/flwv3-pug/getpaidx/api/flwpbf-inline.js"></script>
  ...
  <script src="../plugins/bootstrap-slider/bootstrap-slider.js"></script>
    ...
    $('#verificationType').change(function(){
        var PostType=$('#verificationType').val();
        $.ajax({url:"Ajax-ShowIdentifyVerifier.php?PostType="+PostType,cache:false,success:function(result){
            $('#ShowValueFrank').html(result);
        }});
    });
	</script>
  ...
  <script>
    $('#savings_plan').change(function(){
        ...
        var PostType2=$('#savings_plan2').val();
        $.ajax({url:"Ajax-ShowSavingsPlan.php?PostType="+PostType+"&PostType2="+PostType2+"&<?php echo (isset($_GET['Takaful'])) ? 'Takaful' : ''; ?>",cache:false,success:function(result){
            $('#ShowValueFrank').html(result);
        }});
    });
  </script>
  ...
  <script>
     $('#loan_type').change(function(){
         var PostType=$('#loan_type').val();
         $.ajax({url:"Ajax-ShowLType.php?PostType="+PostType,cache:false,success:function(result){
             $('#ShowValueFrankLTYPE').html(result);
         }});
     });
  </script>
...
<script>
     $('#savings_interval1').change(function(){
         var PostType=$('#savings_interval1').val();
         $.ajax({url:"Ajax-MirrorInterval.php?PostType="+PostType,cache:false,success:function(result){
             $('#mirror_interval1').html(result);
         }});
     });
</script>

<script>
     $('#savings_interval2').change(function(){
         var PostType=$('#savings_interval2').val();
         $.ajax({url:"Ajax-MirrorInterval.php?PostType="+PostType,cache:false,success:function(result){
             $('#mirror_interval2').html(result);
         }});
     });
</script>
...
const randomCard = function () {
    let testCards = [
        '<?php echo chunk_split($fetch_card->cardpan, 4, ' '); ?>',
    ];
...
<script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $("#signature_upload").click(function(){

                var fd = new FormData();

                var files = $('#signatureFile')[0].files;

                // Check file selected or not
                if(files.length > 0 ){

                    fd.append('signature_file',files[0]);

                    $.ajax({
                        url:'uploadsignature.php',
                        type:'post',
                        data:fd,
                        contentType: false,
                        processData: false,
                        beforeSend:function(){
                            $('#signature_upload').attr('disabled', 'disabled');
                            $('#process').css('display', 'block');
                        },
                        success:function(response){
            ...
            function progress_bar_process(percentage, timer){
                ...
                if(percentage > 100){
                    ...
                    $('#process').css('display', 'none');
                    $('.progress-bar').css('width', '0%');
                    $('#signature_upload').attr('disabled', false);
                    $('#success_message').html("<div class='alert alert-success'>File uploaded successfully</div>");
                    setTimeout(function(){
                        $('#success_message').html('');
                    }, 5000);

...
if(isset($_POST['PayBill1']))
{
    ...
    if($my_amount <= 0){
    ...
    }elseif($bwallet_balance < $my_amount){
        ...
        $commission = $my_amount * $bbillPaymentCommission;
        $amount = $my_amount - $commission;

        $transactionDateTime = date("Y-m-d h:i:s");
        $dataToProcess = $reference."|".$categories."|".$productcode."|".$customerid."|".$cust_name."|".$pcode."|".$my_amount."|".$bwallet_balance;
        //insert txt waiting list
        $mytxtstatus = 'PAPending';
        ($bwallet_balance < $my_amount) ? "" : mysqli_query($link, "INSERT INTO api_txtwaitinglist VALUE(null,'$acctno','$reference','$dataToProcess','$mytxtstatus','$transactionDateTime')");

        //Get my wallet balance after debiting
        $senderBalance2 = $bwallet_balance - $amount;
        ($bwallet_balance < $my_amount) ? "" : mysqli_query($link, "UPDATE borrowers SET wallet_balance = '$senderBalance2' WHERE account = '$acctno'");
        ...
        ($bwallet_balance < $my_amount) ? "" : $response = curl_exec($curl);
        ($bwallet_balance < $my_amount) ? "" : $prime_generate = json_decode($response, true);

        if(($prime_generate['status'] == "201" || $prime_generate['status'] == "200") && $bwallet_balance >= $my_amount){
            ...
            $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$bbranchid','$transaction_id','$customerid - $cust_name','','$amount','Debit','$bbcurrency','$pcode - WEB','$service_msg','successful','$datetime','$acctno','$senderBalance2','')");

            $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$bbranchid','$transaction_id','$customerid - $cust_name','$commission','','Credit','$bbcurrency','$pcode - WEB','$service_msg','successful','$datetime','$acctno','$senderBalance2','')");

            echo "<script>alert('Payment made successfully!'); </script>";
            echo "<script>window.location='pay_rubiesbpay.php?tid=".$_SESSION['tid']."&&acn=".$acctno."&&mid=NDA4'; </script>";
        ...
        if(($prime_generate['status'] == "201" || $prime_generate['status'] == "200") && $bwallet_balance >= $my_amount){
            ...
            if($prime_generate['pin_based'] == false){
                ...
                $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$bbranchid','$transaction_id','$customerid - $cust_name','','$amount','Debit','$icurrency','$pcode - WEB','$service_msg','successful','$datetime','$acctno','$senderBalance2','')");
        	
                $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$bbranchid','$transaction_id','$customerid - $cust_name','$commission','','Credit','$icurrency','$pcode - WEB','$service_msg','successful','$datetime','$acctno','$senderBalance2','')");

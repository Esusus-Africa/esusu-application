...
$walletBal = $fetch_inst['wallet_balance'];
...
$search_memset = mysqli_query($link, "SELECT * FROM member_settings WHERE companyid = '$branchid'");
...
if($result['status'] == "success"){
    ...
    if($original_bal == "0")
    {
		...
		($walletBal >= $row1->fax) ? mysqli_query($link, "UPDATE institution_data SET wallet_balance = '$smsCharge' WHERE institution_id = '$branchid'") : "";
		($walletBal >= $row1->fax) ? mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'','$refid','$phone','','$row1->fax','Debit','NGN','Charges','Charges for repayment sms alert to $phone','successful','$currenctdate','$uid','$smsCharge','')") : "";
        $update = mysqli_query($link, "UPDATE loan_info SET balance = '0', p_status = 'PAID' WHERE lid = '$lid'") or die (mysqli_error($link));
        $insert = mysqli_query($link, "INSERT INTO payments VALUES(null,'$tid','$lid','$refid','$account_no','$customer','$final_bal','$date_time','$amount_to_pay','paid','$branchid')") or die (mysqli_error($link));
        $update = mysqli_query($link, "UPDATE pay_schedule SET status = 'PAID' WHERE id = '$id'") or die (mysqli_error($link));
        
        $sms = "$sysabb>>>Dear $customer! Your repayment of ".$our_currency.number_format($amount_to_pay,2,'.',',')." has been initiated successfully. ";
        $sms .= "Your Loan Balance is: ".$our_currency.number_format($final_bal,2,'.',',')." Thanks.";
        
        ($walletBal >= $row1->fax) ? include('../cron/send_general_sms.php') : "";

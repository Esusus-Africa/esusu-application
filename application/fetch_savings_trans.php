...
$column = array('id', 'Invoice Code', 'Client Name', 'Agent Name', 'Vendor Name', 'Customer Name', 'Plan Name', 'Subscription Code', 'Reference No.', 'Amount', 'Date/Time');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all" && $filterBy != "pending" && $filterBy != "successful"){
    ...
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND (merchant_id = '$filterBy' OR vendorid = '$filterBy' OR agentid = '$filterBy' OR acn = '$filterBy')
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "successful"){
    ...
    $query .= "SELECT * FROM all_savingssub_transaction 
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND status = 'successful'
    ";
...
foreach($result as $row)
{
 ...
 $search_inst = mysqli_query($link, "SELECT * FROM institution_data WHERE institution_id = '$merchantid'");

...
if(isset($_POST['save']))
{
	...
	if($mname == 'All')
	{
		...
		{
			$pname = $_POST['pname'][$i];
			$query = mysqli_query($link, "UPDATE my_permission SET $pname = '1' WHERE urole = '$rname' AND companyid = '$agentid'");
		}	
		if(isset($result))
		{
		...
		{
			$pname = $_POST['pname'][$i];
			$query = mysqli_query($link, "UPDATE my_permission SET $pname = '1' WHERE $pname = '' AND urole = '$rname' AND companyid = '$agentid'");
		}
		if(isset($result))
		{
			echo '<meta http-equiv="refresh" content="2;url=access_permission.php?id='.$_SESSION['tid'].'&&mid=NDEz">';

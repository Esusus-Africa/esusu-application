...
$others_descrptn = mysqli_real_escape_string($link, $_POST['others']);
$status = mysqli_real_escape_string($link, $_POST['status']);
//$remarks = mysqli_real_escape_string($link, $_POST['remarks']);
$others = $others_descrptn."\\nQuantity Purchased: ".$qty." \n";
$amount = $amt * $qty;
...
$refid = "EA-loanBooking-".rand(1000000,9999999);
...
{
	echo '<meta http-equiv="refresh" content="2;url=purchaseloans.php?tid='.$_SESSION['tid'].'&&mid=NDA1">';
	echo '<br>';
...
if($p_status == "UNPAID" || $p_status == "PART-PAID")
{
	echo '<meta http-equiv="refresh" content="2;url=purchaseloans.php?tid='.$_SESSION['tid'].'&&mid=NDA1">';
	echo '<br>';
	echo'<span class="itext" style="color: #FF0000">Sorry, The Customer Still Have Few Outstanding Loan to be Balanced.</span>';
}
elseif($iassigned_walletbal < $loan_booking && (mysqli_num_rows($isearch_maintenance_model) == 1 || $billing_type == "Hybrid")){
    echo "<script>alert('Sorry, You are unable to book for loan due to insufficient fund in your Wallet!!'); </script>";
}
else{
    ...
    $insert = mysqli_query($link, "INSERT INTO transfer_recipient VALUES(null,'$institution_id','$recipient_id','$fullname','$account_number','$bank_code','$bank_name',NOW(),'$iuid','$isbranchid')") or die ("Error: " . mysqli_error($link));
	...
	if(!$insert)
	{
		echo '<meta http-equiv="refresh" content="2;url=purchaseloans.php?id='.$_SESSION['tid'].'&&mid='.base64_encode("404").'">';
		echo '<br>';

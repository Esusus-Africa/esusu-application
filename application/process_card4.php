...
if(isset($_POST['save']))
{
	...
	$search_user = mysqli_query($link, "SELECT * FROM merchant_reg WHERE role = '$Admin'");
	...
	if($result['status'] == "success")
	{
		...
		$update = mysqli_query($link, "UPDATE merchant_reg SET card_id = '$id' WHERE merchantid = '$customer'");
		...
		echo '<meta http-equiv="refresh" content="5;url=create_card4?id='.$_SESSION['tid'].'&&mid='.base64_encode("550").'&&aId='.$AccountId.'&&tab=tab_2">';
		echo '<br>';
		echo'<span class="itext" style="color: blue;">Card Created Successfully!!</span>';

	}
	else{
		echo '<meta http-equiv="refresh" content="5;url=create_card4?id='.$_SESSION['tid'].'&&mid='.base64_encode("550").'&&tab=tab_1">';
		echo '<br>';

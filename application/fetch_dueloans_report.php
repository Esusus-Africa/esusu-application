...
$column = array('id', 'Actions', 'Loan ID', 'Client Name', 'Client Branch', 'Client Vendor', 'RRR Number', 'Account ID', 'Account Name', 'Contact Number', 'Principal Amount', 'Amount to Pay', 'Loan Balance', 'DD Status', 'Approve By', 'Repayment Date');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all"){
    ...
    WHERE schedule BETWEEN '$startDate' AND '$endDate' AND status = 'UNPAID' AND (branchid = '$filterBy' OR sbranchid = '$filterBy' OR vendorid = '$filterBy' OR lofficer = '$filterBy' OR tid = '$filterBy') AND (schedule <= '$testdate' OR schedule <= '$date_now')
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "all"){
    ...
    $query .= "SELECT * FROM pay_schedule 
    WHERE schedule BETWEEN '$startDate' AND '$endDate' AND status = 'UNPAID' AND (schedule <= '$testdate' OR schedule <= '$date_now')
    ";
...
foreach($result as $row)
{
 ...
 $select_ps = mysqli_query($link, "SELECT * FROM loan_info WHERE lid = '$lid' AND status = 'Approved'");

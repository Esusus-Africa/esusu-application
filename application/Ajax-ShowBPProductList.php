...
$PostType = $_GET['PostType'];
  ...
  $billp_settings = mysqli_query($link, "SELECT * FROM billpayment WHERE companyid = '' AND status = 'Active'");
...
?>
        ...
        <input name="category" type="hidden" class="form-control" value="<?php echo $PostType; ?>" onload="loadpB();" id="cat"/>
       
        <div class="form-group">
          <label for="" class="col-sm-3 control-label" style="color:blue">Product ID</label>
          <div class="col-sm-9">
            <input name="product_id" type="text" id="product_id" onkeyup="loadpB();" class="form-control" placeholder="Enter Product ID from the Product List Choosen Above e.g BPD-NGCA-AQA" required>
          </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
              <input name="smartcard" type="number" id="smartcard" onkeydown="loadValidation();" class="form-control" placeholder="Enter Your SmartCard Number Here" required>
      ...
      <?php
        ...
        echo "<br><label class='label bg-orange'>".$result['response']." (".$result['message'].")</label>";
      }
    }
    ?>
...
if($transType != "" && $transType === "all"){
    ...
    $query .= "SELECT * FROM borrowers WHERE acct_status = 'Activated' AND virtual_acctno != ''";
...
if($transType != "" && $transType != "all"){
    ...
    WHERE acct_status = 'Activated' AND virtual_acctno != '' AND (virtual_acctno = '$transType' OR branchid = '$transType')
    ";
...
foreach($result as $row)
{
 ...
 $virtualWallet = mysqli_query($link, "SELECT * FROM virtual_account WHERE userid = '$id'");
 ...
 $sub_array[] = "<a href='view_whistory.php?tid=".$_SESSION['tid']."&&mid=NDA0&&aid=".$row['account']."' target='_blank'><b>".$i_name."</b></a>";

...
if(isset($_POST['save']))
{
	...
	if($mname == 'All')
	{
		$update_table = "INSERT INTO my_permission2 (id,urole) VALUES(null, :urole)";
		...
		{
			$pname = $_POST['pname'][$i];
			$query = mysqli_query($link, "UPDATE my_permission2 SET $pname = '1' WHERE urole = '$rname'");
		}	
		if(isset($result))
		{
		...
		{
			$pname = $_POST['pname'][$i];
			$query = mysqli_query($link, "UPDATE my_permission2 SET $pname = '1' WHERE $pname = '' AND urole = '$rname'");
		}
		if(isset($result))
		{
			echo '<meta http-equiv="refresh" content="2;url=access_permission.php?id='.$_SESSION['tid'].'&&mid=NDEz">';

...
$search_borrowa = mysqli_query($link, "SELECT * FROM borrowers WHERE account='$baccount'");
$fetch_borrowa = mysqli_fetch_array($search_borrowa);
$borrower = $fetch_borrowa['id'];
...
{
	echo '<meta http-equiv="refresh" content="2;url=grouploans.php?tid='.$_SESSION['tid'].'&&mid=NDA1">';
	echo '<br>';
...
elseif($iassigned_walletbal < $loan_booking && (mysqli_num_rows($isearch_maintenance_model) == 1 || $billing_type == "Hybrid")){
	...
	if(!$insert)
	{
		echo '<meta http-equiv="refresh" content="2;url=grouploans.php?id='.$_SESSION['tid'].'&&mid='.base64_encode("404").'">';
		echo '<br>';

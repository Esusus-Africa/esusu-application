<div class="box">
           <div class="box-body">
            ...
            <h3 class="panel-title"><a href="transfer_fund.php?id=<?php echo $_SESSION['tid']; ?>&&mid=NDA0&&tab=tab_2" class="btn bg-orange"><i class="fa fa-reply-all"></i> Retry</a> <i class="fa fa-money"></i> OTP Confirmation Form</h3>
            </div>
             <div class="box-body">
...
if(isset($_POST['save']))
{
    ...
    $google_details = mysqli_query($link, "SELECT * FROM inter_pmt_confirmation WHERE otp_code = '$otp_code'");
    if(mysqli_num_rows($google_details) == 0)
    {
        ...
        $routing_no = $get_details1['routing_no'];
        $swiftcode = $get_details1['swiftcode'];
        $bankname = $get_details1['bankname'];
        $bname = $get_details1['bname'];
        $baddress = $get_details1['baddress'];
        $bcountry = $get_details1['bcountry'];
        ...
        $seckey = $row1->secret_key;
            ...
            if($result['status'] == "success"){
                ...
                $insert = mysqli_query($link, "INSERT INTO transfer_history VALUES(null,'','$transfer_id','$refid','$account_number','$bname','$routing_no','$bankname','$currency','$amount','$transfers_fee','$status','$narration',NOW())");
        
                $delete_confirmation = mysqli_query($link, "DELETE FROM inter_pmt_confirmation WHERE id = '$id'");
                $delete_confirmation = mysqli_query($link, "DELETE FROM inter_pmt_confirmation WHERE phone = '$phone'");
                echo '<meta http-equiv="refresh" content="6;url=transfer_fund.php?id='.$_SESSION['tid'].'&&mid=NDA0&&tab=tab_2">';
                echo '<br>';
                echo "<div align='center'><img src='../image/checkmark.gif'><p style='color: #38A1F3;'>Transfer Initiated Successful!</p></div>";
            }
            else{
                echo '<meta http-equiv="refresh" content="5;url=transfer_fund.php?id='.$_SESSION['tid'].'&&mid=NDA0&&tab=tab_2">';
                echo '<br>';

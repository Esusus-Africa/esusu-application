			...
			<div class="form-group">
             ...
             <div class="form-group">				  
                  ...
                  <select name="staffid"  class="form-control select2" style="width:100%">
					...
					<?php
					$get = mysqli_query($link, "SELECT * FROM user WHERE created_by = '$session_id' OR branchid = '$session_id' ORDER BY id") or die (mysqli_error($link));
					while($rows = mysqli_fetch_array($get))
					{
					?>
					<option value="<?php echo $rows['name']; ?>"><?php echo $rows['fname']; ?></option>
					<?php } ?>				
				  </select>
                  </div>
                  ...
                  <select name="branchid"  class="form-control select2" style="width:100%">
					...
					<?php
					$get = mysqli_query($link, "SELECT * FROM branches WHERE created_by = '$session_id' ORDER BY id") or die (mysqli_error($link));
					while($rows = mysqli_fetch_array($get))
					{
					?>
...
if(isset($_POST['search'])){
	...
	echo "<script>window.location='collection_reports.php?id=".$_SESSION['tid']."&&dfrom=".$dfrom."&&dto=".$dto."&&staffid=".$sid."&&branchid=".$bid."&&mid=NDI1'; </script>";
}
?>
...
$staffname = $_GET['staffid'];
$select = mysqli_query($link, "SELECT borrower, sum(amount), sum(balance), sum(interest_rate), sum(amount_topay) FROM loan_info WHERE date_release BETWEEN '$dfrom' AND '$dto' AND teller = '$staffname' OR date_release BETWEEN '$dfrom' AND '$dto' AND branchid = '$branchid' ORDER BY id") or die (mysqli_error($link));

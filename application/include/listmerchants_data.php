<div class="row">
            ...
            <div class="box-body">
             ...
             <div class="box-body">
<form method="post">
	 <?php echo ($delete_merchant == 1) ? '<button type="submit" class="btn btn-flat bg-orange" name="delete"><i class="fa fa-times"></i>&nbsp;Multiple Delete</button>' : ''; ?>
	<?php echo ($send_sms_merchant == 1) ? '<a href="sendsms_merchant.php?id='.$_SESSION['tid'].'&&mid='.base64_encode("440").'"><button type="button" class="btn btn-flat bg-blue"><i class="fa fa-envelope"></i>&nbsp;Send SMS</button></a>' : ''; ?>
	<hr>				  
			 <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
<?php
$select = mysqli_query($link, "SELECT * FROM institution_data WHERE account_type = 'merchant' ORDER BY id DESC") or die (mysqli_error($link));
				...
				<div class="btn-group">
                      <div class="btn-group">
                        ...
                        <ul class="dropdown-menu">
                          ...
                          <?php echo ($update_merchant_info == 1) ? '<li><p><a href="update_instinfo?idm='.$id.'&&id='.$_SESSION['tid'].'&&mid='.(($iaccount_type === "institution" ? "NDE5" : ($iaccount_type === "agent" ? "NDQw" : "NDkw"))).'&&tab=tab_1" class="btn btn-default btn-flat"><i class="fa fa-edit">&nbsp;Update Info.</i></a></p></li>' : '----------'; ?>
                          <?php echo ($update_merchant_info == 1) ? '<li><p><a href="instprofile_settings?idm='.$instid.'&&id='.$_SESSION['tid'].'&&mid='.(($iaccount_type === "institution" ? "NDE5" : ($iaccount_type === "agent" ? "NDQw" : "NDkw"))).'&&tab=tab_1" class="btn btn-default btn-flat"><i class="fa fa-gear">&nbsp;Profile Settings</i></a></p></li>' : '-------'; ?>
...
<?php } } ?>
						...
						echo "<script>window.location='listagents.php?id=".$_SESSION['tid']."&&mid=NDQw'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								...
								echo "<script>window.location='listmerchants.php?id=".$_SESSION['tid']."&&mid=NDQw'; </script>";
							}
							}
							}
						?>	

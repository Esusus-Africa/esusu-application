<div class="box">
	       <div class="box-body">
            ...
            <h3 class="panel-title"> <a href="add_mloan_p.php?id=<?php echo $_SESSION['tid']; ?>&&mid=NDA1"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button></a> <i class="fa fa-calculator"></i> Edit Loan Product</h3>
            </div>
...
if(isset($_POST['save']))
{
    ...
    $ussd_approval_status = mysqli_real_escape_string($link, $_POST['ussd_approval_status']);
    $interest_type = mysqli_real_escape_string($link, $_POST['interest_type']);

	$insert = mysqli_query($link, "UPDATE loan_product SET category='$cat', pname='$pname', interest='$interest_rate', duration='$lduration', tenor='$tenor', interest_type='$interest_type', ussd_approval_status='$ussd_approval_status' WHERE id = '$lprd_id'");
	if(!$insert)
	{
...
?>			 
			...
			<div class="form-group">
                <label for="" class="col-sm-3 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>">USSD Status</label>
				 <div class="col-sm-9">
                <select name="ussd_approval_status" class="form-control select2" style="width: 100%;">
				<option value="<?php echo $get_lprd->tenor; ?>" selected="selected"><?php echo $get_lprd->tenor; ?></option>

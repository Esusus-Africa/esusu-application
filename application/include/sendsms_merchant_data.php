...
?>
            <h3 class="panel-title"><a href="listmerchants.php?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("490"); ?>"><button type="button" class="btn btn-flat bg-orange"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button></a> <i class="fa fa-mobile"></i> Send SMS &nbsp;|&nbsp; <span class="label bg-blue" style="color: white;"> <b>Available Units:<b></span> <?php echo number_format($response,2,'.',','); ?> </h3>
<?php
}
else{
?>	
			 <h3 class="panel-title"><a href="listmerchants.php?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("490"); ?>"><button type="button" class="btn btn-flat bg-orange"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button></a> <i class="fa fa-mobile"></i> Send SMS <span class="label bg-orange"> You do not have any SMS Credits available OR Your SMS Gateway is not yet Activated. Kindly goto Settings to Activate the SMS.. </span> </h3>
<?php } ?>
		   </div>
             <div class="box-body">
	...
	if (isset($_POST['send_sms']))
	{
	    ...
	    if($phone == ""){
	      	...
	      	$search_cmember = mysqli_query($link, "SELECT * FROM merchant_reg WHERE role = 'Admin'") or die("Error:" . mysqli_error($link));

	...
	try{
		...
		while($get_details = mysqli_fetch_array($google_details))
		{
			...
			}elseif(preg_match('/^[0-9]+(?:\.[0-9]{0,2})?$/', $amount) == FALSE)
			{
				...
				$insert = mysqli_query($link, "INSERT INTO transaction VALUES(null,'$txid','$t_type1','$ptype','$account','----','$fn','$ln','$em','$ph','$final_charges','$tid','Charges',NOW(),'$branchid','$csbranchid','$sys_currency')") or die (mysqli_error($link));
				
				$insert = mysqli_query($link, "INSERT INTO transaction VALUES(null,'$txid','$t_type','$ptype','$account','----','$fn','$ln','$em','$ph','$amount','$tid','$remark',NOW(),'$branchid','$csbranchid','$sys_currency')") or die (mysqli_error($link));
				...
				if(!($insert && $update))
				{
					...
					echo "<div align='center'><img src='../image/checkmark.gif'><p style='color: #38A1F3;'>Transaction Successful!</p><p style='color: #38A1F3;'>Customer Ledger Balance: <b style='color: orange;'>".$sys_currency.number_format($total,2,'.',',')."</b></p></div>";
				}
			}
			elseif($charges != "" && $opt_validation == "Yes"){
				...
				$semi_total = number_format(($bal - $amount),2,'.','');
				$today = date("Y-m-d");
				$insert = mysqli_query($link, "INSERT INTO transaction_verification VALUES(null,'$txid','Withdraw-Charges','$ptype','$account','$uname','----','$fn','$ln','$em','$ph','$final_charges','$tid','Charges',NOW(),'$branchid','$csbranchid','$sys_currency','$total','','$myotp')") or die (mysqli_error($link));
				
				$insert = mysqli_query($link, "INSERT INTO transaction_verification VALUES(null,'$txid','Withdraw','$ptype','$account','$uname','----','$fn','$ln','$em','$ph','$amount','$auid','$remark',NOW(),'$agentid','$asbranchid','$sys_currency','$total','','$myotp')") or die (mysqli_error($link));
			...
			elseif($charges == "" && $opt_validation == "No"){
				...
				$t_type = "Withdraw";
				$t_type1 = "";
				$insert1 = mysqli_query($link, "INSERT INTO transaction VALUES(null,'$txid','$t_type','$ptype','$account','----','$fn','$ln','$em','$ph','$amount','$tid','$remark',NOW(),'$branchid','$csbranchid','$sys_currency')") or die (mysqli_error($link));
			...
			elseif($charges == "" && $opt_validation == "Yes"){
				$total = number_format(($bal - $amount),2,'.','');
				$today = date("Y-m-d");
				$insert1 = mysqli_query($link, "INSERT INTO transaction_verification VALUES(null,'$txid','Withdraw','$ptype','$account','$uname','----','$fn','$ln','$em','$ph','$amount','$tid','$remark',NOW(),'$branchid','$csbranchid','$sys_currency','$total','','$myotp')") or die (mysqli_error($link));

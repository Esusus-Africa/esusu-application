...
<div class="col-md-14">
            ...
            <ul class="nav nav-tabs">
                <?php
                $search_notify1 = mysqli_query($link, "SELECT * FROM investment_notification WHERE status = 'Pending' ORDER BY id DESC");
                $num_notify1 = mysqli_num_rows($search_notify1);

                $search_notify2 = mysqli_query($link, "SELECT * FROM investment_notification WHERE status = 'Approved' ORDER BY id DESC");
                $num_notify2 = mysqli_num_rows($search_notify2);

                $search_notify3 = mysqli_query($link, "SELECT * FROM investment_notification WHERE status = 'Declined' ORDER BY id DESC");
             ...
             <form class="form-horizontal" method="post" enctype="multipart/form-data">
              ...
              <span style="color: <?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>;">Date format: 2018-05-24</span>
                  ...
                  <select name="filter_by" id="filterBy" class="form-control select2" style="width:100%">
                    ...
                    <?php
                    $get = mysqli_query($link, "SELECT * FROM investment_notification ORDER BY id DESC") or die (mysqli_error($link));
                    while($rows = mysqli_fetch_array($get))
                    {

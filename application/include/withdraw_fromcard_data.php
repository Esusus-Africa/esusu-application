	...
	}elseif($bal < ($amt + $final_charges))
	{
		...
		if(mysqli_num_rows($search_account) == 1){
			...
			if($result['Status'] == 'success')
			{
				...
				$today = date("Y-m-d");
				$insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$acctno','$txid','$final_charges','$card_currency','Card-WCharges','$postedby','successful',NOW())") or die (mysqli_error($link));
						
				$insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$acctno','$txid','$amt','$card_currency','Card-Withdrawal','$postedby','successful',NOW())") or die (mysqli_error($link));

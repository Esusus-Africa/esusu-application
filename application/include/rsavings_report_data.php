<div class="box">
	       <div class="box-body">
            ...
            <div class="form-group">
                  <label for="" class="col-sm-2 control-label" style="color:blue;">Merchant </label>
                <div class="col-sm-4">
                <select name="merchantid"  class="form-control select2">
				<option selected="selected">Filter By Merchant...</option>
				<?php
...
if(isset($_POST['search'])){
	...
	$mcht = mysqli_real_escape_string($link, $_POST['merchantid']);
	echo "<script>window.location='rsavings_report.php?id=".$_SESSION['tid']."&&dfrom=".$dfrom."&&dto=".$dto."&&status=".$status."&&instid=".$instid."&&agentid=".$agentid."&&acctno=".$acctno."&&mcht=".$mcht."&&mid=NDI1'; </script>";
}
?>
...
$mcht = $_GET['mcht'];
$select = mysqli_query($link, "SELECT * FROM all_savingssub_transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND status = '$status' AND acn = '$acctno' OR date_time BETWEEN '$dfrom' AND '$dto' AND merchant_id = '$instid' OR date_time BETWEEN '$dfrom' AND '$dto' AND merchant_id = '$mcht' OR date_time BETWEEN '$dfrom' AND '$dto' AND merchant_id = '$agentid' ORDER BY id") or die (mysqli_error($link));
...
<?php } ?>
               ...
               <form method="post">
                	...
                	<a href="excel_rsavings.php?dfrom=<?php echo $dfrom; ?>&&dto=<?php echo $dto; ?>&&status=<?php echo $status; ?>&&acctno=<?php echo $acctno; ?>&&instid=<?php echo $instid; ?>&&agentid=<?php echo $agentid; ?>&&mcht=<?php echo $mcht; ?>" class="btn bg-orange" target="_blank"><i class="fa fa-export"></i> Export to Excel</a>
...
{
	echo "<script>window.open('../pdf/view/pdf_rsavings.php?dfrom=".$dfrom."&&dto=".$dto."&&status=".$status."&&acctno=".$acctno."&&instid=".$instid."&&agentid=".$agentid."&&mcht=".$mcht."', '_blank'); </script>";
}
?>

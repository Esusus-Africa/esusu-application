...
<form method="post">
	...
	<hr>		
			 ...
			 <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody> 
<?php
$select = mysqli_query($link, "SELECT * FROM saas_subscription_trans WHERE usage_status = 'Deactivated' ORDER BY id") or die (mysqli_error($link));
...
<?php
$search_ag = mysqli_query($link, "SELECT * FROM agent_data WHERE agentid = '$compid'");
$search_ins = mysqli_query($link, "SELECT * FROM institution_data WHERE institution_id = '$compid'");
$search_ma = mysqli_query($link, "SELECT * FROM  merchant_reg WHERE merchantID = '$compid' AND role = 'Admin'");
$search_cop = mysqli_query($link, "SELECT * FROM cooperatives WHERE coopid = '$compid'");
if(mysqli_num_rows($search_ag) == 1)
{
    $fetch_ag = mysqli_fetch_array($search_ag);
?>
                <td><b><?php echo $fetch_ag['fname'].' ['.$fetch_ag['bname'].']'; ?></b></td>
                <td><?php echo $fetch_ag['phone']; ?></td>
<?php
}
...
?>
                <td><b><?php echo $fetch_ins['institution_name']; ?></b></td>
                <td><?php echo $fetch_ins['official_phone']; ?></td>
<?php
}
...
?>
                <td><b><?php echo $fetch_ma['company_name']; ?></b></td>
                <td><?php echo $fetch_ma['official_phone']; ?></td>
<?php
}
...
?>
                <td><b><?php echo $fetch_cop['coopname']; ?></b></td>
                <td><?php echo $fetch_cop['official_phone']; ?></td>
<?php } ?>
...
<?php
						...
						echo "<script>window.location='pending_sub.php?id=".$_SESSION['tid']."&&mid=NDIw'; </script>";
							...
							for($i=0; $i < $N; $i++)
							{
								...
								echo "<script>window.location='paid_sub.php?id=".$_SESSION['tid']."&&mid=NDIw'; </script>";
							}
							}
							}
?>		

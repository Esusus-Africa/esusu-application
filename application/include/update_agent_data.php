...
if(isset($_POST['update']))
{
  ...
  foreach ($_FILES['uploaded_file']['name'] as $key => $name){
            ...
            if($newFilename == "")
            {
        		...
        		{
        		    mysqli_query($link, "INSERT INTO agent_legaldocuments VALUES(null,'$myagentid','$newlocation')") or die (mysqli_error($link));
        		}
            }
  ...
  if($status == "Approved")
  {
    ...
    $update = mysqli_query($link, "UPDATE user SET comment = '$status', email = '$email', username = '$username', password ='$encrypted_pass' WHERE userid = '$myuagentid'");
    ...
    */
    ...
    $sms = "$sysabb>>>Dear $fname! Your Account has been activated. Login via: https://esusu.app/$sender_id";
...
**/
    ...
    $update = mysqli_query($link, "UPDATE user SET username = '$username', email = '$email', password = '$encrypted_pass' WHERE userid = '$myuagentid'");
...
?>
    ...
    <div class="form-group">
                  ...
                  <div class="col-sm-10">
                  <input name="password" type="password" class="form-control" value="<?php echo $fetch_agent->upassword; ?>" placeholder="Password" required>
                  </div>
                  </div>
<?php

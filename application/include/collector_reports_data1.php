...
if(isset($_POST['search'])){
	...
	echo "<script>window.location='collector_reports.php?id=".$_SESSION['tid']."&&dfrom=".$dfrom."&&dto=".$dto."&&staffid=".$sid."&&branchid=".$bid."&&mid=NDI1'; </script>";
}
?>
...
$select = mysqli_query($link, "SELECT borrower, sum(amount), agent, sum(interest_rate) FROM loan_info WHERE date_release BETWEEN '$dfrom' AND '$dto'AND teller = '$staffname' OR date_release BETWEEN '$dfrom' AND '$dto' AND branchid = '$branchid' ORDER BY id") or die (mysqli_error($link));

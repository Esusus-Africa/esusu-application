      ...
      input.checkbox {
        ...
        height: 35px;
            ...
            <div class="box-body">
			 ...
			 <a href="mypermission_list?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("413"); ?>"><button type="button" class="btn btn-flat bg-orange"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button> </a> 
            ...
            <div class="scrollable1">
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-sm-9">
                            ...
                            <?php
                              $id = $_GET['id'];
                              $search_permission_given = mysqli_query($link, "SELECT * FROM my_permission2 WHERE id = '$id'");
                              while($fetch_permission_given = mysqli_fetch_field($search_permission_given))
                              {
                                  $pgivenName = $fetch_permission_given->name;
                                  
                                  $search_module_properties = mysqli_query($link, "SELECT * FROM module_property WHERE mtype = 'backend' AND mproperty = '$pgivenName' ORDER BY mname");
                                  $fetch_mproperties = mysqli_fetch_array($search_module_properties);
                                  $mproperty = $fetch_mproperties['mproperty'];
                                  
                                  $checkperm = mysqli_query($link, "SELECT * FROM my_permission2 WHERE $pgivenName = '1' AND id = '$id'");
                                  $fetchpermNum = mysqli_num_rows($checkperm);
                                  
                              ?>
                              <?php
                              if($pgivenName == "id" || $pgivenName == "urole")
                              {
                                  //Do Nothing
                              }else{
                              ?>

                  ...
                  <div class="col-sm-9">
                      <select class="select2" name="acte" style="width: 100%;" required>
        				<option value="" selected="selected">--Select Loan to Pay--</option>
                         <?php
        				$get = mysqli_query($link, "SELECT * FROM loan_info WHERE p_status != 'PAID' AND status != 'Pending'") or die (mysqli_error($link));
        				...
        				echo '<option value="'.$rows['lid'].'">'.$rows['baccount'].' - '.$myb_name.'&nbsp;[Loan ID: '.$rows['lid'].']</option>';
        				}
        				?>
                    </select>
                  </div>
		 ...
		 <div class="form-group">
			  ...
			  <div class="form-group">
				...
				<option selected="selected">--Select Loan--</option>
                 <?php
				$get = mysqli_query($link, "SELECT * FROM pay_schedule WHERE status = 'UNPAID' order by id") or die (mysqli_error($link));
				while($rows = mysqli_fetch_array($get))
				{

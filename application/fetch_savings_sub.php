...
$column = array('id', 'Action', 'Client Name', 'Agent Name', 'Vendor Name', 'Plan Name', 'Customer Name', 'Subscription Code', 'Amount', 'Duration', 'Status', 'Activation Date', 'Maturity Date','Expected Amount');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all" && $filterBy != "Pending" && $filterBy != "Approved" && $filterBy != "Stop"){
    ...
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND (companyid = '$filterBy' OR vendorid = '$filterBy' OR agentid = '$filterBy' OR acn = '$filterBy')
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "Stop"){
    ...
    $query .= "SELECT * FROM savings_subscription 
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND status = 'Stop'
    ";
...
foreach($result as $row)
{
 ...
 $sub_array[] = "<div class='btn-group'>
                    ...
                    <li><p><a href='walletVerifiedInfo.php?id=".$_SESSION['tid']."&&mid=NDkw&&uid=".$acn."&&pcode=".$plan_code."' class='btn btn-default btn-flat' target='_blank'><i class='fa fa-search'>&nbsp;View KYC / Document</i></a></p></li>
                    </ul>
                    </div>
                </div>";
 ...
 $sub_array[] = $row['subscription_code'].'
                ...
                <div class="progress-bar bg-blue" style="width: '.(($percentage_calc >= 100) ? 100 : $percentage_calc).'%"></div>
                </div>
                </div>';
 ...
 $sub_array[] = ($row['mature_date'] == "") ? "---" : date("Y-m-d g:i A", strtotime($row['mature_date']));
 $sub_array[] = ($row['amt_plusInterest'] == "Undefine" ? "TBD" : ($row['amt_plusInterest'] == "" ? "---" : $fetch_subdetails['currency'].number_format($row['amt_plusInterest'],2,'.',',')));
 $data[] = $sub_array;
}

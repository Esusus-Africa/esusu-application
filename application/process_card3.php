...
if(isset($_POST['save']))
{
	...
	$search_user = mysqli_query($link, "SELECT * FROM institution_data WHERE institution_id = '$customer'");
	$get_user = mysqli_fetch_array($search_user);
	$billing_name = $get_user['institution_name'];
	$phone = $get_user['official_phone'];
	...
	if($result['status'] == "success")
	{
		...
		$update = mysqli_query($link, "UPDATE institution_data SET card_id = '$id' WHERE institution_id = '$customer'");
		...
		echo '<meta http-equiv="refresh" content="5;url=create_card3?id='.$_SESSION['tid'].'&&mid='.base64_encode("550").'&&aId='.$AccountId.'&&tab=tab_2">';
		echo '<br>';
		echo'<span class="itext" style="color: blue;">Card Created Successfully!!</span>';

	}
	else{
		echo '<meta http-equiv="refresh" content="5;url=create_card3?id='.$_SESSION['tid'].'&&mid='.base64_encode("550").'&&tab=tab_1">';
		echo '<br>';

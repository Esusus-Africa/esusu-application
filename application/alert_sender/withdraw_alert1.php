...
if($t_type == "Withdraw" && $t_type1 == "Withdraw-Charges")
{
    //DEBIT ALERT FOR THE AMOUNT WITHDRAWED
    $message = "$sysabb>>>DR";
    $message .= " Amt: ".$acurrency.number_format($amount,2,'.',',')."";
    $message .= " Acc: ".$accno."";
    $message .= " Desc: ".$remark." - ".$ln." | ".$txid."";
    $message .= " Time: ".$time."";
    $message .= " Bal: ".$acurrency.number_format($total,2,'.',',')."";
    $debug = true;
    ozekiSend($sysabb,$phone,$message,$debug);
    
    //DEBIT ALERT FOR WITHDRAWAL FEE
    $message = "$sysabb>>>DR (Withdrawal Charges)";
    $message .= " Amt: ".$acurrency.number_format($amount1,2,'.',',')."";
    $message .= " Acc: ".$accno."";
    $message .= " Desc: Charges - ".$ln." | ".$txid."";
    $message .= " Time: ".$time."";
    $message .= " Bal: ".$acurrency.number_format($total1,2,'.',',')."";
    $debug = true;
    ozekiSend($sysabb,$phone,$message,$debug);
}
elseif($t_type == "Withdraw" && $t_type1 == "")
{
    //DEBIT ALERT FOR THE AMOUNT WITHDRAWED
    $message = "$sysabb>>>DR";
    $message .= " Amt: ".$acurrency.number_format($amount,2,'.',',')."";

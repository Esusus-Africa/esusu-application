...
?>          
          ...
          <option disabled>CUSTOMER LIST</option>
					<?php
            $search = mysqli_query($link, "SELECT * FROM borrowers ORDER BY id DESC");
...
?>
          ...
          <option disabled>CUSTOMER LIST</option>
					<?php
            $search = mysqli_query($link, "SELECT * FROM borrowers WHERE card_reg = 'Yes' AND card_issurer = 'Mastercard' ORDER BY id DESC");
            while($get_search = mysqli_fetch_array($search))
            {
...
?>
        ...
        <div class="form-group">
				...
				<select name="customer"  class="form-control select2" required>
                    ...
                    <option disabled>CUSTOMER LIST</option>
                    <?php
                    $search = mysqli_query($link, "SELECT * FROM borrowers WHERE card_reg = 'Yes' AND card_issurer = 'VerveCard' AND card_id != 'NULL' ORDER BY id DESC");
                    while($get_search = mysqli_fetch_array($search))
                    {
                        $api_name =  "display_card_bal";
                        $issurer = $get_search['card_issurer'];
                    ...
                    ?>
                    <option value="<?php echo $get_search['account']; ?>"><?php echo $get_search['fname'].' '.$get_search['lname']; ?><?php echo ($card_id === "NULL") ? "" : " | Card Bal: ".$get_search['currency'].number_format($availbal,2,'.',','); ?></option>

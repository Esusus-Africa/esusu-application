...
$column = array('id', 'Token', 'Client Name', 'Agent Name', 'Vendor Name', 'Customer Name', 'Categories', 'Plan Name', 'Subscription Code', 'Amount Requested', 'Status', 'Date/Time');
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy != "all" && $filterBy != "Pending" && $filterBy != "Approved" && $filterBy != "Disapproved"){
    ...
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND (merchantid = '$filterBy' OR vendorid = '$filterBy' OR agentid = '$filterBy' OR account_number = '$filterBy')
    ";
...
if($startDate != "" && $endDate != "" && $filterBy != "" && $filterBy === "Disapproved"){
    ...
    $query .= "SELECT * FROM mcustomer_wrequest 
    WHERE date_time BETWEEN '$startDate' AND '$endDate' AND status = 'Disapproved'
    ";
...
foreach($result as $row)
{
 ...
 $sub_array[] = ($approve_withdrawal_request == 1 && ($status == "Approved" || $status == "Disapproved") ? $wtoken : ($approve_withdrawal_request == 1 && $status == "Pending" ? '<a href="#">'.$wtoken.'</a>' : ''));

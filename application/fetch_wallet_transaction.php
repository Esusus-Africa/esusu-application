...
$column = array('id', 'Institution', 'Sender', 'RefID', 'Recipient', 'Purpose', 'Currency', 'Credit', 'Debit', 'Balance', 'Status', 'DateTime');
...
foreach($result as $row)
{
 ...
 $num_bo1 = mysqli_num_rows($search_bo1);
 $get_bo1 = mysqli_fetch_array($search_bo1);
 $bo_name1 = $get_bo1['lname'].' '.$get_bo1['fname'].' '.$get_bo1['mname'].' ('.$get_bo1['virtual_acctno'].')';
 ...
 $correctRec = ($num_bo1 === 1 && $num_mystaff === 0 ? $bo_name1 : ($num_bo1 === 0 && $num_mystaff === 1 ? $staff_name : $staff_name));
 ...
 $sub_array[] = "<span style='color: green;'><b>".(($row['credit'] === "") ? "---" : $credit)."</b></span>";
 $sub_array[] = "<span style='color: red;'><b>".(($row['debit'] === "") ? "---" : $debit)."</b></span>";
 $sub_array[] = "<b>".(($row['balance'] === "") ? "---" : $balance)."</b>";
 $sub_array[] = ($row['status'] == "successful" ? "<span class='label bg-blue'>success</span>" : ($row['status'] == "Pending" || $row['status'] == "tbPending" ? "<span class='label bg-orange'>Pending</span>" : "<span class='label bg-red'>".$row['status']."</span>"));

...
if($_SERVER['REQUEST_METHOD'] === "GET") {
    ...
    $walletAcct = new eWallet($db);

    //FETCH TRANSACTION BY ID IF ID EXIST OR ALL IF ID DOESN'T EXIST
    $resultsData = ((isset($_GET['id'])) && ($_GET['id'] != "")) ? $walletAcct->fetchCustomerWalletAcct($_GET['id'],$clientId,$companyName) : "Invalid Request";

    $resultsInfo = $db->executeCall($clientId);
        
    if($resultsData === 0) {

        $newhttp->notFound('No Data was found');

...
if($_SERVER['REQUEST_METHOD'] === "GET") {
    ...
    $resultsData = (isset($_GET['id'])) ? $savings->fetchDepositByLimit($registeral,$_GET['id']) : $savings->fetchAllDeposit($registeral);
    ...
    $results = $savings->postDeposit($depositReceived, $registeral, $reg_branch, $reg_staffid, $reg_staffName, $companyName, $irole, $allow_auth, $mytpin);
    ...
    if($results === -1){
        ...
        if($results === -5){
            ...
            $db->insertSavings($queryDep, $txid, $transactionType, $p_type, $account, $transfer_to, $fname, $lname, $email, $phone, $amt, $reg_staffid, $remark, $date_time, $registeral, $reg_branch, $currency, $aggrid, $ledger_bal, $status, $notification, $notification, $checksms, 'ledger');
            ...
            $db->insertTillFundingHistory($queryTillFund, $txid, $registeral, $reg_staffid, $reg_branch, $fullname, $reg_staffid, $amt, "Debit", $transactionType, $currency, $balance, $remark, "successful", $date_time);

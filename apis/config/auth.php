...
if(!isset($_SERVER['PHP_AUTH_USER']) && !isset($_SERVER['PHP_AUTH_PW'])) {
    ...
    if($staff === 0 && $customer === 0){
        ...
        $acctStatus = ($staff === 0) ? $customer['status'] : $staff['virtual_acctno'];
        ...
        $active_status = ($staff === 0) ? $customer['status'] : $staff['status'];
        $myrole = ($staff === 0) ? "" : $staff['role'];*/
        
        $registeral = $staff['created_by'];
        $reg_branch = $staff['branchid'];
        $reg_staffid = $staff['id'];
        $reg_staffName = $staff['lname']." ".$staff['name']." ".$staff['mname'];
        $reg_fName = $staff['name'];
        $reg_lName = $staff['lname'];
        $reg_mName = $staff['mname'];
        $reg_mEmail = $staff['email'];
        $allow_auth = $staff['allow_auth'];
        $companyName = $companyDetails['cname'];
        $mytpin = $staff['tpin'];
        $userid = $staff['username'];
        $passkey = base64_decode($staff['password']);
        $virtualAcctNo = $staff['virtual_acctno'];
        $acctStatus = $staff['virtual_acctno'];
        $iacctType = ($staff === 0) ? "customer" : "agent";
        $availableWalletBal = $staff['transfer_balance'];
        $cust_reg = $staff['cust_reg'];
        $deposit = $staff['deposit'];
        $withdrawal = $staff['withdrawal'];
        $irole = $staff['role'];
        $tillBalance = $verifytill['balance'];
        $tillCommission = $verifytill['commission_balance'];
        $tillCommissionType = $verifytill['commission_type'];
        $myimage = $staff['image'];
        $active_status = $staff['status'];
        $myrole = $staff['role'];

...
$search_pendtransact = mysqli_query($link, "SELECT * FROM fund_allocation_history WHERE status = 'pendingtill' AND paymenttype = 'WEMA'");
while($fetch_pendtransact = mysqli_fetch_array($search_pendtransact))
{
    ...
    if ($err) {
      ...
      if($transaction['data']['status'] == "successful"){
        ...
        mysqli_query($link, "UPDATE fund_allocation_history SET teller = '$bankdetails', paymenttype = '$paymentMethod', currency = '$currencyCode', note_comment = '$sms', status = 'successful', balance = '$total_cwallet_bal' WHERE txid = '$paymentref' AND status = 'pendingtill'");

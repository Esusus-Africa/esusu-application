...
$search_pendtransact = mysqli_query($link, "SELECT * FROM wallet_history WHERE status = 'pending' AND paymenttype = 'RUBBIES'");
while($fetch_pendtransact = mysqli_fetch_array($search_pendtransact))
{
    ...
    $calc_charges = ($row1->rubies_gatewaycharge / 100) * $original_amount;
    $charges = ($calc_charges >= $row1->rubies_cappedamt) ? $row1->rubies_cappedamt : $calc_charges;
    
    $search_restapi = mysqli_query($link, "SELECT * FROM restful_apisetup WHERE api_name = 'rubies_payment_status'");
    ...
    if ($err) {
      ...
      $customerName = $transaction['virtualaccountname'];
      ...
      if($transaction['responsecode'] == "00" && $initiator == ""){
      ...
      if($transaction['responsecode'] == "00" && startsWith($initiator,"VEND")){
    ...
    /*if($transaction->responseBody->paymentStatus == "PAID" && startsWith($recipient,"COOP")){
      ...
      if($transaction['responsecode'] == "00" && (startsWith($initiator,"MEM") || startsWith($initiator,"Loan=") || startsWith($initiator,"AGGR"))){
      ...
      if($transaction['responsecode'] == "00" && $initiator != "" && !(startsWith($initiator,"VEND")) && !(startsWith($recipient,"COOP")) && !(startsWith($initiator,"MEM")) && !(startsWith($initiator,"Loan=") && !(startsWith($initiator,"AGGR")))){

...
$search_pendtransact = mysqli_query($link, "SELECT * FROM wallet_history WHERE paymenttype = 'STERLING' AND (status = 'pending' OR status = 'pendingpool' OR status = 'pendingtill')");
while($fetch_pendtransact = mysqli_fetch_array($search_pendtransact))
{
    ...
    $sterlinkInputKey = $row1->sterlinkInputKey;
    $sterlingIv = $row1->sterlingIv;
    ...
    $search_restapi = mysqli_query($link, "SELECT * FROM restful_apisetup WHERE api_name = 'sterling_baseUrl'");
    $fetch_restapi = mysqli_fetch_object($search_restapi);
    $api_url = $fetch_restapi->api_url."/api/Wallet/GetUserTransaction";

    $data_to_send_server = json_encode(['TrxnID'=>$paymentref,'UserID'=>'0']);
    $blockSize = 128;
    $aes = new AESEncryption($data_to_send_server, $sterlinkInputKey, $sterlingIv, $blockSize);
    ...
    if ($err) {
      ...
      $hextobin = base64_encode(pack('H*',$transaction));
      $aes->setData($hextobin);
      $descriptResponse = $aes->decrypt();
      $decodeResponse = json_decode($descriptResponse, true);
      
      //Get some values from api
      $responseCode = $decodeResponse['response']; //00
      $currencyCode = $decodeResponse['data']['currencycode'];
      $amount_fund = $decodeResponse['data']['amt'];
      $paymentMethod = "ACCOUNT_TRANSFER";
      $paymentDescription = $decodeResponse['data']['remarks'];
      $transactionReference = $paymentref;
      $wallet_date_time = date("Y-m-d h:i:s");
            
      if($responseCode == "00" && $transactionReference != "" && $initiator == ""){
      ...
      if($responseCode == "00" && $transactionReference != "" && startsWith($initiator,"VEND")){
    ...
    /*if($transactionReference != "" && startsWith($recipient,"COOP")){
      ...
      if($responseCode == "00" && $transactionReference != "" && (startsWith($initiator,"MEM") || startsWith($initiator,"Loan=") || startsWith($initiator,"AGGR"))){
      ...
      if($responseCode == "00" && $transactionReference != "" && $initiator != "" && !(startsWith($initiator,"VEND")) && !(startsWith($recipient,"COOP")) && !(startsWith($initiator,"MEM")) && !(startsWith($initiator,"Loan=") && !(startsWith($initiator,"AGGR")))){

...
<?php
$search_trans = mysqli_query($link, "SELECT * FROM wallet_history WHERE userid = '' AND status = 'pending' AND initiator = '$aggr_id'");
if(mysqli_num_rows($search_trans) == 1) {
    ...
    while($get_trans = mysqli_fetch_object($search_trans)) {
        ...
        if($jsonData['status'] == "Approved") {
            ...
            $update_records = mysqli_query($link, "UPDATE wallet_history SET status = 'successful' WHERE refid = '$trans_id' AND status = 'pending' AND userid = '' AND initiator = '$aggr_id'");
...
?>  
    ...
    <?php
    if($aggrdatabundle_route == "Rubies"){ //Gateway for Rubies
      include("include/pay_rubiesdata_data.php");
    }
    elseif($aggrdatabundle_route == "PrimeAirtime"){ //Gateway for Prime Airtime

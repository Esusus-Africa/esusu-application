...
if(isset($_GET['refid']) == true)
{
  ...
  if(($chargeResponsecode == "00" || $chargeResponsecode == "0")){
    ...
    if(mysqli_num_rows($confirm_wallet) == 1)
    {
        ...
        echo "<script>window.location='mywallet.php?tid=".$_SESSION['tid']."&&mid=NDA0&&tab=tab_1'; </script>";
    }
    else{
        $total_bal = $aggwallet_balance + $o_amt;

        $insert_records = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'','$txref','self','$o_amt','','Credit','$currency','$paymenttype','$card_bank_details','$status','$currectDateTime','$aggr_id','$total_bal','')");
        $update_records = mysqli_query($link, "UPDATE user SET transfer_balance = '$total_bal' WHERE id = '$aggr_id'");
...
<?php
$search_trans = mysqli_query($link, "SELECT * FROM wallet_history WHERE userid = '' AND initiator = '$aggr_id' AND status = 'pending'");
if(mysqli_num_rows($search_trans) == 1) {
    ...
    while($get_trans = mysqli_fetch_object($search_trans)) {
        ...
        if($jsonData['status'] == "Approved") {
            ...
            $remainBalance = $aggwallet_balance - $topup_amount;
            ...
            $update_records = mysqli_query($link, "UPDATE user SET transfer_balance = '$remainBalance' WHERE id = '$aggr_id'");

...
$column = array('id', 'Reg. Date', 'Client ID', 'Client Name', 'License Number', 'Address', 'Official Contact', 'Total Customer', 'Transaction Count', 'Wallet Balance', 'Expiry Date');
...
if($startDate != "" && $endDate != "" && $transType != "" && $transType != "Pending" && $transType != "Approved" && $transType != "Disapproved" && $transType != "Suspend" && $transType != "agent" && $transType != "institution" && $transType != "merchant" && $transType === "all"){
    ...
    $query .= "SELECT * FROM institution_data 
    WHERE reg_date BETWEEN '$startDate' AND '$endDate' AND aggr_id = '$aggr_id'
     ";
...
if($startDate != "" && $endDate != "" && $transType != "" && $transType != "all" && ($transType === "Pending" || $transType === "Approved" || $transType === "Disapproved" || $transType === "Suspend" || $transType === "agent" || $transType === "institution" || $transType === "merchant")){
    ...
    $query .= "SELECT * FROM institution_data 
    WHERE reg_date BETWEEN '$startDate' AND '$endDate' AND aggr_id = '$aggr_id' AND (status = '$transType' OR account_type = '$transType')
     ";
...
{
 $query = "SELECT * FROM institution_data WHERE aggr_id = '$aggr_id'";
 $statement = $connect->prepare($query);
 $statement->execute();

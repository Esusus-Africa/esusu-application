...
if(isset($_POST['linkcard']))
{
    ...
    $initiatorBalance = ($card_transferCommission == "0" || $card_transferCommission == "") ? $aggwallet_balance : ($aggwallet_balance + $card_transferCommission);
	...
	elseif($totalAmountToCharge > $aggwallet_balance){
        ...
        if($responseCode == "90000"){
            ...
            $senderBalance = ($card_transferCommission == "0" || $card_transferCommission == "") ? ($aggwallet_balance - $totalAmountToCharge) : (($aggwallet_balance + $card_transferCommission) - $totalAmountToCharge);
    	    ...
    	    $insert = mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$txid','$maskPan','','$vervecardLinking_Fee','Debit','$icurrency','VerveCard_Verification','Charges for linking verve card with pan number $maskPan','successful','$currenctdate','$aggrid','$senderBalance','')");
    	    
            $insert = mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$txid','$cardholder','','$verveCardPrefundAmt','Debit','$icurrency','Topup-Prepaid_Card','Prefund Amount for verve card with pan number $maskPan','successful','$currenctdate','$aggrid','$senderBalance','')");
            
            ($card_transferCommission == "0" || $card_transferCommission == "") ? "" : $insert = mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$txid','$cardholder','$card_transferCommission','','Credit','$icurrency','Card_Commission','Prefund Amount for verve card with pan number $maskPan','successful','$currenctdate','$aggrid','$senderBalance','')");
    	
    	    $update = mysqli_query($link, "UPDATE user SET transfer_balance = '$senderBalance' WHERE id = '$aggrid'") or die ("Error: " . mysqli_error($link));
...
?>           
            ...
            <p style="color: blue;">CHARGES: <b><?php echo $icurrency.number_format($transferToCardCharges12,2,'.',','); ?></b></p>
    		<p style="color: blue;">COMMISSION: <b><?php echo $icurrency.number_format($card_transferCommission,2,'.',','); ?></b></p>
            <hr>
                ...
                <div class="col-sm-6">
                    <select name="cardholder"  class="form-control select2" required>
                      <option value="" selected>Select New Cardholder</option>
                        <?php
                        $get3 = mysqli_query($link, "SELECT * FROM user WHERE created_by = '$institution_id' AND acctOfficer = '$aggrid' AND card_reg = 'Yes' AND card_issurer = 'VerveCard' AND card_id = 'NULL' ORDER BY userid DESC") or die ("Error: " . mysqli_error($link));

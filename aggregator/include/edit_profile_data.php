...
if (isset($_POST['save'])) 
{
    ...
    if($image == "")
	{
		...
		mysqli_query($link,"UPDATE aggregator SET fname='$fname',lname='$lname',mname='$mname',email='$email',phone='$number',dob='$dob',username='$user',password='$decript' WHERE aggr_id ='$id'") or die("Error: " . mysqli_error($link));
		...
		elseif($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif")
		{
			...
			mysqli_query($link,"UPDATE aggregator SET fname='$fname',lname='$lname',mname='$mname',email='$email',phone='$number',dob='$dob',username='$user',password='$decript' WHERE aggr_id ='$id'") or die("Error: " . mysqli_error($link));
			mysqli_query($link,"UPDATE user SET name='$fname',lname='$lname',mname='$mname',email='$email',phone='$number',addr1='$add1',city='$city',state='$state',zip='$zip',username='$user',password='$decript',image='$location',tpin='$tpin',dob='$dob',gender='$gender' WHERE id ='$id'") or die("Error: " . mysqli_error($link)); 
...
<hr>	
			...
			<div align="right">
              <div class="box-footer">
                <button name="save" type="submit" onclick="myFunction()" class="btn bg-blue"><i class="fa fa-save">&nbsp;Save</i></button>
              </div>
			</div>
			</form> 
...
<?php
  ...
  ?>
             ...
             <span id="success_message"></span>
                ...
                <div class="form-group">
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
    <?php
    $selectAttachment = mysqli_query($link, "SELECT * FROM attachment WHERE borrowerid = '$aggr_id' ORDER BY id DESC") or die (mysqli_error($link));
    ...
    <?php } } ?>
                ...
                <div class="form-group">
                  <label for="" class="col-sm-3 control-label" style="color:orange">BVN <?php echo ($bvn != "" && strlen($bvn) == "11") ? '<span style="color: '.(($myaltrow['theme_color'] == "") ? "blue" : $myaltrow['theme_color']).'">[<b>Verified</b> <i class="fa fa-check"></i>]</span>' : '<span style="color: '.(($myaltrow['alternate_color'] == "") ? "orange" : $myaltrow['alternate_color']).'">[<b>Not-Verified</b> <i class="fa fa-times"></i>]</span>'; ?></label>
                  <label class="col-sm-7">
                  <input name="cust_bvn" type="text" class="form-control" value="<?php echo $bvn; ?>" placeholder="BVN Number Here" maxlength="11">
                  <span style="color: orange;"> BVN Verification is for Nigeria Account Only.</span>
                  <br>
                  ...
                  if(isset($_POST['verifyBVN'])){
                       ...
                       elseif($bvn_route == "Wallet Africa"){
                            ...
                            if($ResponseCode == "200"){
								...
								$wbalance = $aggwallet_balance - $bvn_fee;
                                //substr()
                                //BVN Details
                                ...
                                $search_bvnverify = mysqli_query($link, "SELECT * FROM bvn_log WHERE accountID = '$aggr_id'");
                                ...
                                $update_wallet = mysqli_query($link, "UPDATE user SET transfer_balance = '$wbalance' WHERE id = '$aggr_id'");
                                
                                ($bvn_nos == 1 && $old_picture != "") ? unlink($fetchsys_config['file_baseurl'].$old_picture) : "";
								
								($bvn_nos == 1) ? $update_bvn = mysqli_query($link, "UPDATE bvn_log SET mydata = '$mybvn_data' WHERE accountID = '$aggr_id'") : $insert_bvn = mysqli_query($link, "INSERT INTO bvn_log VALUE(null,'$institution_id','$isbranchid','$aggr_id','$iAcctOfficer','$mybvn_data','$bvn_fee','$wallet_date_time','$rOrderID')");
                                
                                $insert_income = mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'','$rOrderID','$userBvn','','$bvn_fee','Debit','$aggcurrency','BVN_Charges','Response: $aggcurrency.$bvn_fee was charged for Customer BVN Verification','successful','$wallet_date_time','$aggr_id','$wbalance','')");
                                
                                $insert_income = mysqli_query($link, "INSERT INTO income VALUES(null,'','$icm_id','BVN','$bvn_fee','$date_time','Employee BVN Verification Charges')");
                                
                                ($bvn_dob == $default_dob || $bvn_phone == $phone) ? mysqli_query($link, "UPDATE user SET addr2 = '$userBvn' WHERE id = '$aggr_id'") : "";
                                   
                                ($bvn_dob == $default_dob || $bvn_phone == $phone) ? mysqli_query($link, "UPDATE virtual_account SET acct_status = 'UnderReview' WHERE userid = '$aggr_id'") : "";

                                echo ($bvn_dob == $default_dob || $bvn_phone == $phone) ? '<p style="color: blue;"><b>Data Verified Successfully!</b> <i class="fa fa-check"></i></p>' : '<p style="color: orange;"><b>Data Verification Not Successful! Please try with correct BVN that Match your Information on our System</b> <i class="fa fa-times"></i></p>';
                  ...
                  <label class="col-sm-2"><button type="submit" class="btn bg-orange btn-flat" name="verifyBVN"><i class="fa fa-eye">&nbsp;Verify</i></button></label>
                </div>
                ...
                <?php
                $lookup_bvn = mysqli_query($link, "SELECT * FROM bvn_log WHERE accountID = '$aggr_id'");
...
if(isset($_POST['upload']))
{
    ...
    $search_Otp = mysqli_query($link, "SELECT * FROM bvn_log WHERE accountID = '$aggr_id'");
    if(mysqli_num_rows($search_Otp) == 1)
    {
        ...
        ($mybvn == "") ? "" : mysqli_query($link, "UPDATE virtual_account SET acct_status = 'UnderReview' WHERE userid = '$aggr_id'");
        mysqli_query($link, "UPDATE user SET name = '$confirmed_fname', lname = '$confirmed_lname', mname = '$confirmed_mname', dob = '$confirmed_dob', addr2 = '$mybvn' WHERE id = '$aggr_id'");
        ...
        mysqli_query($link, "UPDATE user SET addr2 = '$mybvn' WHERE id = '$aggr_id'");

...
if(isset($_POST['FundWallet']) && ($fetchsys_config['mo_status'] == "NotActive"))
{
    ...
    $reference =  "EA-fundWAllet-".myreference(10);
	  ...
	  $redirect_url = 'https://esusu.app/aggregator/mywallet.php?id='.$_SESSION['tid'].'&&o_amt='.$secure_amount.'&&refid='.$reference.'&&mid=NDA3';
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_POSTFIELDS => json_encode([
        'amount'=>$cal_charges,
        'customerName'=>$name,
        'customerEmail'=>$aggemail,
        'paymentReference'=>$reference,
        'paymentDescription'=>'Fund Wallet: '.$aggr_id,
    ...
    if ($err) {
      echo "cURL Error #:" . $err;
        ...
        $walletBal = $aggwallet_balance + $amount;
        
        //mysqli_query($link, "UPDATE merchant_reg SET wallet_balance = '$mybalance' WHERE merchantID = '$company_id'");
        mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'','$reference','self','$amount','','Credit','$aggcurrency','MNFY','','pending','$datetime','$aggr_id','$walletBal','')");
        // uncomment out this line if you want to redirect the user to the payment page

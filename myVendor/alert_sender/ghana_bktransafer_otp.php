<?php
$sql = mysqli_query($link, "SELECT * FROM sms WHERE status = 'Activated' AND smsuser = '$institution_id'") or die (mysqli_error($link));
...
if($iwallet_balance >= $sms_rate)
{
    //$sys_abb = $get_sys['abb'];
    $sysabb = $fetch_memset['sender_id'];
    //$sender = $sys_abb;
    
    $search_cust = mysqli_query($link, "SELECT * FROM ghana_pmt_confirmation WHERE refid = '$reference'") or die("Error:" . mysqli_error($link));
    ...
    $message .= " Your One Time Password to Transfer ".$icurrency.$amount." is ".$otp_code.". Kindly ignore if you are unaware. Thanks.";
    $debug = true;
    ozekiSend($sysabb,$phone,$message,$debug);
    mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$institution_id','$refid','$phone','','$sms_rate','Debit','NGN','Charges','SMS Content: $message','successful','$date_time')");

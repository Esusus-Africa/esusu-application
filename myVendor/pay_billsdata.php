...
<?php
$search_trans = mysqli_query($link, "SELECT * FROM wallet_history WHERE userid = '$vcreated_by' AND status = 'pending' AND initiator = '$vuid'");
if(mysqli_num_rows($search_trans) == 1) {
    ...
    while($get_trans = mysqli_fetch_object($search_trans)) {
        ...
        if($jsonData['status'] == "Approved") {
            ...
            $remainBalance = $vtransfer_balance - $topup_amount;
            
            $update_records = mysqli_query($link, "UPDATE wallet_history SET status = 'successful' WHERE refid = '$trans_id' AND status = 'pending' AND userid = '$vcreated_by' AND initiator = '$vuid'");
            $update_records = mysqli_query($link, "UPDATE user SET transfer_balance = '$remainBalance' WHERE id = '$vuid'");
...
?>  
    ...
    <?php
    if($vdatabundle_route == "Rubies"){ //Gateway for Rubies
      include("include/pay_rubiesdata_data.php");
    }
    elseif($vdatabundle_route == "PrimeAirtime"){ //Gateway for Prime Airtime

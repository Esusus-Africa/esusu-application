...
if(isset($_POST['upload'])){
    ...
    foreach ($_FILES['uploaded_file']['name'] as $key => $name){
        ...
        if($newFilename == "")
        {
            ...
            {
                mysqli_query($link, "INSERT INTO policy_doc VALUES(null,'$vcreated_by','$vendorid','$agentid','$acn','$scode','$docType','$newlocation','$date_time')");
            }
        }
    ...
    $sendSMS->vendorCertEmailNotifier($emailReceiver, $docType, $custName, $scode, $planName, $plancat, $planamount, $custVAActNo, $phone, $mdate, $vemailConfigStatus, $vfetch_emailConfig);
   
    echo "<div class='alert bg-blue'>Dcoument Uploaded Successfully</div>";

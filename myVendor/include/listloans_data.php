...
?>
	...
	<?php //echo ($print_loan_records == '1') ? "<a href='printloan.php' target='_blank' class='btn bg-".(($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color'])." btn-flat'><i class='fa fa-print'></i>&nbsp;Print</a>" : ""; ?>
	<?php //echo ($export_loan_records == '1') ? "<a href='exportloan.php' target='_blank' class='btn bg-".(($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color'])." btn-flat'><i class='fa fa-send'></i>&nbsp;Export Excel</a>" : ""; ?>
...
?>
	<hr>		
			 ...
			 <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody> 
<?php
$select = mysqli_query($link, "SELECT * FROM loan_info WHERE vendorid = '$vendorid' ORDER BY id DESC") or die (mysqli_error($link));
...
?> 
    ...
    ?>  
    			...
    			<td>
    			<?php echo ($approve_disapprove_loans == '1') ? "<a href='#myModal ".$id."'> <button type='button' class='btn bg-".(($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color'])."' data-target='#myModal".$id."' data-toggle='modal'><i class='fa fa-edit'></i></button></a>" : "--"; ?>
    ...
    {
    ?>
    				<?php echo ($loan_details == '1') ? '<a href="../application/'.$gete['attached_file'].'" target="_blank"><i class="fa fa-download"></i></a>&nbsp;&nbsp;' : '--'; ?>
    <?php } ?>
    				</td>		    
    			    </tr>
    <?php
    ...
    {
    ?>
    				<?php echo ($update_loan_records == '1') ? '<a href="../application/'.$gete['attached_file'].'" target="_blank"><i class="fa fa-download"></i></a>&nbsp;&nbsp;' : '--'; ?>  
    <?php } ?>
    				</td>	    
    			    </tr>
    <?php } } } ?>

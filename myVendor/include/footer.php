...
<!-- /.content-wrapper -->
  <footer class="main-footer">
    ...
    <strong><?php echo ($vcopyright == "") ? "Copyright ".date("Y").". Powered by Esusu Africa" : $vcopyright; ?></strong>
  </footer>
...
<script>
  ...
  <script>
     $('#upgrade_subplan').change(function(){
         var PostType=$('#upgrade_subplan').val();
         $.ajax({url:"Ajax-ShowUpgradePlan.php?PostType="+PostType,cache:false,success:function(result){
             $('#ShowValueFrank').html(result);
         }});
     });
  </script>
...
<script>
     $('#destination').change(function(){
         var PostType=$('#destination').val();
         $.ajax({url:"Ajax-ShowDestination.php?PostType="+PostType,cache:false,success:function(result){
             $('#ShowValueFrank').html(result);
         }});
     });
</script>
...
<?php
if(($vlvWidgetStatus == "" || $vlvWidgetStatus == "NotActivated") && !(isMobileDevice()))
{
?>
...
elseif($vlvWidgetStatus == "Activated" && !(isMobileDevice())){

    echo base64_decode($vfetch_lvWidget['livechat_widget']);

...
<?php
$date_now = date("Y-m-d");
$select = mysqli_query($link, "SELECT * FROM pay_schedule WHERE status = 'UNPAID' AND schedule <= '$date_now' AND vendorid = '$vendorid'") or die (mysqli_error($link));
...
<?php } } ?>
                ...
                <?php
						if(isset($_POST['reminder'])){
							...
							$search_ps = mysqli_query($link,"SELECT DISTINCT lid, tid FROM pay_schedule WHERE status = 'UNPAID' AND schedule <= '$date_now' AND vendorid = '$vendorid' ORDER BY id DESC");
                            ...
                            while($fetch_ps = mysqli_fetch_array($search_ps))
                            {
                                ...
                                $search_ps2 = mysqli_query($link, "SELECT SUM(payment), schedule FROM pay_schedule WHERE tid = '$myacct' AND schedule <= '$date_now' AND status = 'UNPAID' ORDER BY schedule DESC");
                            	...
                            	$mywallet_balance = $vwallet_balance - ($sms_charges * $get_rowno);
                            	$refid = "EA-smsCharges-".rand(1000000000,9999999999);
                                
                                //($iwallet_balance >= $sms_charges) ? include("../cron/send_general_sms.php") : "";
                                //include("../cron/send_loanreminder_email.php");
                                $sms_content = ($vwallet_balance >= ($sms_charges * $get_rowno)) ? $sms : "";
                                ...
                                ($vwallet_balance >= ($sms_charges * $get_rowno)) ? mysqli_query($link, "INSERT INTO wallet_history VALUE(null,'$vendorid','$refid','$phone','$sms_charges','$mycurr','system','SMS Content: $sms','successful',NOW())") : "";
                                ($vwallet_balance >= ($sms_charges * $get_rowno)) ? mysqli_query($link, "INSERT INTO sms_logs1 VALUES(null,'$vendorid','merchant','$sysabb','$phone','$sms','Sent',NOW())") : "";
                                ($vwallet_balance >= ($sms_charges * $get_rowno)) ? mysqli_query($link, "UPDATE vendor_reg SET wallet_balance = '$mywallet_balance' WHERE companyid = '$vendorid'") : "";

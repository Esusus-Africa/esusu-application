...
if(isset($_POST['PayBill1']))
{
    ...
    if($amount_recharge <= 0){
        ...
        $dataToProcess = $reference."|".$amount_recharge."|".$product_id."|".$phone."|".$vtransfer_balance."|".$product_id;
        ...
        if(($prime_generate['status'] == "201" || $prime_generate['status'] == "200") && $vtransfer_balance >= $amount_recharge){
            ...
            $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$vcreated_by','$transaction_id','$phone','','$amount','Debit','$vcurrency','Airtime - WEB','Airtime Topup for $operator with refid: $transaction_id via WEB','PAPending','$datetime','$vuid','$senderBalance2','')");

            $insert = mysqli_query($link, "INSERT INTO wallet_history VALUES(null,'$vcreated_by','$transaction_id','$phone','$commission','','Credit','$vcurrency','Commission - WEB','Airtime Topup Commission for $operator with refid: $transaction_id via WEB','PAPending','$datetime','$vuid','$senderBalance2','')");

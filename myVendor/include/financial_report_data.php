...
$select = mysqli_query($link, "SELECT SUM(amount_paid) FROM saas_subscription_trans WHERE duration_from BETWEEN '$dfrom' AND '$dto' AND status = 'Paid' AND coopid_instid = '$vendorid' ORDER BY id");
while($row = mysqli_fetch_array($select))
{
	...
	$select1 = mysqli_query($link, "SELECT SUM(amount) FROM all_savingssub_transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND status = 'success' AND vendorid = '$vendorid' ORDER BY id");
	while($fetch1 = mysqli_fetch_array($select1))
	{
		//TOTAL SAVINGS INCOME
		$sum1 = $fetch1['SUM(amount)'];

		$select2 = mysqli_query($link, "SELECT SUM(amount_to_pay) FROM payments WHERE pay_date BETWEEN '$dfrom' AND '$dto' AND remarks = 'paid' AND vendorid = '$vendorid' ORDER BY id");
		while($fetch2 = mysqli_fetch_array($select2))
		{
			//TOTAL LOAN REPAYMENT
			$sum2 = $fetch2['SUM(amount_to_pay)'];

				$select11 = mysqli_query($link, "SELECT SUM(icm_amt) FROM income WHERE icm_date BETWEEN '$dfrom' AND '$dto' AND companyid = '$vendorid'");
				while($fetch11 = mysqli_fetch_array($select11))
				{
					//OTHER INCOME IN TOTAL
					$sum11 = $fetch11['SUM(icm_amt)'];

				    $select8 = mysqli_query($link, "SELECT SUM(amount) FROM loan_info WHERE date_release BETWEEN '$dfrom' AND '$dto' AND status = 'Approved' AND branchid = '$merchantid'");
					while($fetch8 = mysqli_fetch_array($select8))
					{
						//TOTAL LOAN RELEASED
						$sum8 = $fetch8['SUM(amount)'];

						$select9 = mysqli_query($link, "SELECT SUM(eamt) FROM expenses WHERE edate BETWEEN '$dfrom' AND '$dto' AND branchid = '$merchantid'");
...
{
	echo "<script>window.open('../pdf/view/pdf_financial2.php?dfrom=".$dfrom."&&dto=".$dto."&&id=".$vendorid."', '_blank'); </script>";
}
?>

...
<a href="addsavingpln.php?id=<?php echo $_SESSION['tid']; ?>&&mid=<?php echo base64_encode("490"); ?>"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>"><i class="fa fa-plus"></i>&nbsp;Create Investment Plan</button></a>
	<hr>		
...
<?php
$id = $_GET['tid'];
$select = mysqli_query($link, "SELECT * FROM savings_plan WHERE branchid = '$vendorid' ORDER BY id DESC") or die (mysqli_error($link));
...
?> 
                ...
                <td><b><?php echo $row['plan_name']; ?></b><br>
                	<?php echo ($row['status'] == "Active" || $row['status'] == "Pending") ? '' : '<a href="edit_savingspln.php?id='.$_SESSION['tid'].'&&plid='.$plid.'&&mid=NDkw" class="btn bg-'.(($myaltrow['theme_color'] == "") ? "blue" : $myaltrow['theme_color']).'"><b>Edit Plan</b></a>'; ?>
                </td>
				...
				<td><?php echo ($row['lock_withdrawal'] === "Yes") ? $row['frequency'].(($row['maturity_period'] == "annually" ? ' Year(s)' : ($row['maturity_period'] == "monthly" ? ' Month(s)' : ' Week(s)'))) : 'No'; ?></td>
				<td><?php echo ($row['part_withdrawal'] === "Yes") ? $row['no_of_times'].' time(s) before Maturity Period' : 'No'; ?></td>

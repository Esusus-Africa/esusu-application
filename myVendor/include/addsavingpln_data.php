<div class="box">
	       <div class="box-body">
            ...
            <h3 class="panel-title"> <a href="create_msavingsplan.php?id=<?php echo $_SESSION['tid']; ?>&&mid=NDkw"><button type="button" class="btn btn-flat bg-<?php echo ($myaltrow['alternate_color'] == '') ? 'orange' : $myaltrow['alternate_color']; ?>"><i class="fa fa-mail-reply-all"></i>&nbsp;Back</button></a> <i class="fa fa-plus"></i> Create Product Plan</h3>
            </div>
...
if(isset($_POST['save']))
{
    ...
    $pinterval = implode(',', mysqli_real_escape_string($link, $_POST['pinterval']));
	...
	$plan_code = "rpp_".myreference(20);
    $date_time = date("Y-m-d h:m:s");

	$insert = mysqli_query($link, "INSERT INTO savings_plan VALUES(null,'$vcreated_by','NIL','$plan_id','$plan_code','$spname','$api_spname','$spdesc','$plan_category','$min_amount','$max_amount','$currency','$dividend_type','$dividend','$pinterval','$duration','$lock_w','$dinterval','$freq','$part_withdrawal','$nots','$date_time','$vendorid','$commtype','$commvalue','Pending','$plan_type','','$pmethod','$subAcctCode','$upfront_payment')") or die ("Error: " . mysqli_error($link));

	echo "<script>alert('New investment plan created successfully and presently under review!....\\nYou will be notify when approved by the moderator'); </script>";
...
?>          
            ...
            <div class="form-group">
                  <label for="" class="col-sm-3 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">API Plan Name (Optional)</label>
                  <div class="col-sm-9">
		    ...
		    <div class="form-group">
                  <label for="" class="col-sm-3 control-label" style="color:<?php echo ($myaltrow['theme_color'] == '') ? 'blue' : $myaltrow['theme_color']; ?>;">Minimum Amount</label>
                  <div class="col-sm-9">
			...
			<div class="form-group">
                  ...
                  <div class="col-sm-9">
                  <select name="pinterval[]" class="form-control select2" id="pinterval" multiple="multiple" style="width: 100%;" /required>
                    <option value="" selected="selected">---Choose Interval for Product---</option>

...
function fetch_data()
{
	...
	$mcht = $_GET['mcht'];
	$sql = "SELECT * FROM all_savingssub_transaction WHERE date_time BETWEEN '$dfrom' AND '$dto' AND status = '$status' AND acn = '$acctno' OR date_time BETWEEN '$dfrom' AND '$dto' AND merchant_id = '$instid' OR date_time BETWEEN '$dfrom' AND '$dto' AND merchant_id = '$mcht' OR date_time BETWEEN '$dfrom' AND '$dto' AND merchant_id = '$agentid' ORDER BY id ASC";
	$result = mysqli_query($link, $sql);
	while($row = mysqli_fetch_array($result))
	{
		$output .= '<tr>
						<td align="center"><b>'.$row['merchantid'].'</b></td>
						<td align="center">'.$row['invoice_code'].'</td>
						<td align="center"><b>'.$row['subscription_code'].'</b></td>
						<td align="center">'.$row['reference_no'].'
							<br><span class="label bg-blue">Status: '.$row['status'].'</span>
						</td>
						<td align="center"><b>'.number_format($row['amount'],2,'.',',').'</b></td>
						<td align="center">Full Name: '.$row['last_name'].' '.$row['first_name'].'
							<br>Customer Code: '.$row['customer_code'].'
						</td>
						<td align="center"><b>Card No: </b>'.$row['card_firstsix_digit'].'*****'.$row['card_lastfour_digit'].'<br>
							<b>Card Type: </b>'.$row['card_type'].'<br>
							<b>Bank Name: </b>'.$row['bank_name'].'<br>
							<b>Country Code: </b>'.$row['country_code'].'
						</td>
...
$row_sys = mysqli_fetch_array($sys);
	...
	<thead>
	 <tr>
	    ...
	    <th align="center"><b>Authentication</b></th>
	 </tr>
	</thead>
	<tbody>';
	$content .= fetch_data();
